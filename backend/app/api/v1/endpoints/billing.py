...
async def create_subscription(
    ...
    subscription = billing_service.create_subscription(
        str(current_user.organization_id), subscription_data
    )
...
async def update_subscription(
    ...
    subscription = billing_service.update_subscription(
        subscription_id, str(current_user.organization_id), update_data
    )
...
@router.post(
    "/subscription/{subscription_id}/cancel", response_model=SubscriptionResponse
)
async def cancel_subscription(
    ...
    subscription = billing_service.cancel_subscription(
        subscription_id, str(current_user.organization_id), immediate=immediate
    )
...
async def create_checkout_session(
    ...
    session = billing_service.create_checkout_session(
        str(current_user.organization_id), checkout_data
    )
...
async def create_billing_portal_session(
    ...
    session = billing_service.create_billing_portal_session(
        str(current_user.organization_id), portal_data
    )
...
async def get_invoices(
    ...
    invoices = billing_service.get_organization_invoices(
        str(current_user.organization_id), limit=limit
    )
...
async def get_billing_overview(
    ...
    billing_service = BillingService(db)
    overview = billing_service.get_billing_overview(str(current_user.organization_id))

    # Convert to response model
    return BillingOverviewResponse(
        subscription=(
            SubscriptionResponse(**overview["subscription"])
            if overview["subscription"]
            else None
        ),
        upcoming_invoice=(
            InvoiceResponse(**overview["upcoming_invoice"])
            if overview.get("upcoming_invoice")
            else None
        ),
...
@router.post(
    "/usage", response_model=UsageRecordResponse, status_code=status.HTTP_201_CREATED
)
async def record_usage(
    ...
    usage_record = billing_service.record_usage(
        str(current_user.organization_id), usage_data
    )
...
async def get_usage_summary(
    ...
    billing_service = BillingService(db)
    usage = billing_service.get_usage_summary(str(current_user.organization_id))
    return usage
...
async def check_usage_limits(
    ...
    billing_service = BillingService(db)
    limits_check = billing_service.check_usage_limits(str(current_user.organization_id))
    return limits_check

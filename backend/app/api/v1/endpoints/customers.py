...
class CustomerRecoveryRequest(BaseModel):
    ...
    trigger_context: Optional[Dict[str, Any]] = Field(None, description="Context that triggered recovery")
    execute_immediately: bool = Field(False, description="Execute actions immediately vs schedule")
...
async def get_customers(
    ...
    query = select(Customer).where(Customer.organization_id == org_context.organization_id)
    ...
    for customer in customers:
        ...
        customer_dict = {
            ...
            "first_review_date": stats.first_review_date.isoformat() if stats.first_review_date else None,
            "risk_score": float(customer.churn_risk_score) if customer.churn_risk_score else 0.0,
            "risk_level": customer.risk_level.lower() if customer.risk_level else "low",
            "churn_probability": float(customer.bad_review_likelihood) if customer.bad_review_likelihood else 0.0,
...
async def recover_customer(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Recovery process failed: {str(e)}")

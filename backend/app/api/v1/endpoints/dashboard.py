...
from app.services.realtime_metrics import RealTimeMetricsService, websocket_manager
...
async def get_dashboard_metrics(
    time_range: str = Query(
        "30d", pattern="^(7d|30d|90d|1y)$", description="Time range for metrics"
    ),
    ...
    **Caching:**
    ...
    try:
        ...
        metrics = await dashboard_service.get_comprehensive_metrics(
            organization_id=organization_id, time_range=time_range
        )
...
async def get_kpis(
    ...
    try:
        ...
        return KPIResponse(
            kpis=kpis, time_range=time_range, generated_at=datetime.utcnow().isoformat()
        )
    ...
    try:
        ...
        if metric_type:
            if metric_type == "sentiment":
                ...
                trends = {
                    "review_volume_over_time": trends.get("review_volume_over_time", [])
                }
            elif metric_type == "recovery":
                trends = {
                    "recovery_actions_over_time": trends.get(
...
async def get_average_rating(
    ...
    try:
        ...
        avg_rating, total_reviews = await kpi_service.calculate_average_rating(
            organization_id=organization_id, start_date=start_date
        )

        # Get previous period for comparison
        prev_start = start_date - timedelta(days=days)
        prev_avg_rating, prev_total_reviews = (
            await kpi_service.calculate_average_rating(
                organization_id=organization_id,
...
async def get_websocket_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {"websocket_stats": stats, "timestamp": datetime.utcnow().isoformat()}

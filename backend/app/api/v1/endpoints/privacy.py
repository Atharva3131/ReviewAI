...
from app.core.privacy_policy import get_privacy_policy_service, PolicyType
from app.core.gdpr_compliance import get_gdpr_service, GDPRRights
from app.core.consent_management import get_consent_service, ConsentType, ConsentMethod
...
async def get_privacy_policy(
    ...
    try:
        ...
        if format == "json":
            policy = privacy_service.get_policy_for_display(org, PolicyType.PRIVACY_POLICY, "json")
            if not policy:
                raise HTTPException(status_code=404, detail="Privacy policy not found")
            return policy
        else:
            policy = privacy_service.get_policy_for_display(org, PolicyType.PRIVACY_POLICY, "html")
            if not policy:
                raise HTTPException(status_code=404, detail="Privacy policy not found")
...
async def get_terms_of_service(
    ...
    try:
        ...
        if format == "json":
            policy = privacy_service.get_policy_for_display(org, PolicyType.TERMS_OF_SERVICE, "json")
            if not policy:
                raise HTTPException(status_code=404, detail="Terms of service not found")
            return policy
        else:
            policy = privacy_service.get_policy_for_display(org, PolicyType.TERMS_OF_SERVICE, "html")
            if not policy:
                raise HTTPException(status_code=404, detail="Terms of service not found")
            return HTMLResponse(content=policy["content"])
...
async def get_cookie_policy(
    ...
    try:
        ...
        if format == "json":
            policy = privacy_service.get_policy_for_display(org, PolicyType.COOKIE_POLICY, "json")
            if not policy:
                raise HTTPException(status_code=404, detail="Cookie policy not found")
            return policy
        else:
            policy = privacy_service.get_policy_for_display(org, PolicyType.COOKIE_POLICY, "html")
            if not policy:
                raise HTTPException(status_code=404, detail="Cookie policy not found")
            return HTMLResponse(content=policy["content"])
...
async def get_processing_purposes(
    org: str = Query(..., description="Organization ID")
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve processing purposes")
...
async def get_consent_banner_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve consent configuration")
...
async def withdraw_consent(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=400, detail="Cannot withdraw this consent type")
...
async def generate_compliance_report(
    ...
    try:
        ...
        from app.core.compliance_reporting import get_compliance_reporting_service, ReportType
...
async def get_consent_report(
    ...
    try:
        ...
        return BaseResponse(
            success=True,
            data=report,
            message="Consent report generated successfully"
        )
...
async def create_policy(
    ...
    try:
        ...
        if not policy_type or not content:
            raise HTTPException(
                status_code=400,
                detail="policy_type and content are required"
            )

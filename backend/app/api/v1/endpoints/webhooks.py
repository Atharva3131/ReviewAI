...
from app.services.external.google_reviews import GoogleReviewsWebhookSimulator, create_google_reviews_service
...
async def google_reviews_webhook(
    ...
    try:
        ...
        background_tasks.add_task(
            _process_google_reviews_webhook,
            webhook_data,
            db
        )
...
async def _process_google_reviews_webhook(webhook_data: Dict[str, Any], db: AsyncSession):
...
async def _handle_new_review(review_data: Dict[str, Any], db: AsyncSession):
    ...
    try:
        ...
        if processed_review:
            logger.info(f"Successfully processed new Google review: {review_data.get('review_id')}")
...
async def crm_interaction_webhook(
    ...
    try:
        ...
        # Process webhook in background
        background_tasks.add_task(
            _process_crm_interaction_webhook,
            webhook_data,
            db
        )
...
async def get_service_errors(service_name: str, limit: int = 50):
    ...
    try:
        ...
        logger.error(f"Error getting service errors for {service_name}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def _process_crm_interaction_webhook(webhook_data: Dict[str, Any], db: AsyncSession):
...
async def _handle_crm_interaction_created(interaction_data: Dict[str, Any], db: AsyncSession):
    ...
    try:
        ...
        risk_service = CustomerRiskService(db)
        await risk_service.update_customer_context_from_interaction(customer_id, interaction_data)
        
        logger.info(f"CRM interaction created: {interaction_type} for customer {customer_id}")

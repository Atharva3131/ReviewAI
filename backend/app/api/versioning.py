            ...
            raise HTTPException(
                ...
                headers={
                    "X-Supported-Versions": ", ".join(
                        [v.value for v in cls.SUPPORTED_VERSIONS.keys()]
                    )
                },
            )
...
class VersionedResponse:
    ...
    def format_response(data: Any, version: APIVersion) -> Dict[str, Any]:
        ...
        return {"data": data, "version": version.value, "timestamp": int(time.time())}

    @staticmethod
    def format_error(
        error: str, version: APIVersion, details: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        if version == APIVersion.V1:
            ...
            return {
                "error": {"message": error, "details": details or {}},
                "meta": {
                    "version": version.value,
                    "timestamp": int(time.time()),
                    "request_id": None,
        ...
        try:
            ...
            return JSONResponse(
                ...
                content=VersionedResponse.format_error(
                    e.detail, APIVersionManager.DEFAULT_VERSION
                ),

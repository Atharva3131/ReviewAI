...
class ComplianceReport:
    ...
    def add_issue(
        self, severity: str, title: str, description: str, recommendation: str = None
    ):
...
class ComplianceReportingService:
    ...
    async def generate_gdpr_compliance_report(
        ...
        report.summary = {
            ...
            "gdpr_requests_period": len(
                [
                    r
                    for r in gdpr_service.pending_requests.values()
                    if period_start <= r.created_at <= period_end
                ]
            ),
            "compliance_status": (
                "compliant"
                if report._calculate_compliance_score() >= 80
                else "non_compliant"
            ),
        }
        ...
        logger.info(
            f"Generated GDPR compliance report {report_id} for organization {organization_id}"
        )
    ...
    async def generate_consent_management_report(
        ...
        await self._add_consent_withdrawal_metrics(
            report, consent_service, organization_id
        )
        ...
        report.summary = {
            ...
            "average_consent_rate": self._calculate_average_consent_rate(
                consent_report
            ),
        }
        ...
        self.reports[report_id] = report
        logger.info(
            f"Generated consent management report {report_id} for organization {organization_id}"
        )
    ...
    async def generate_data_retention_report(
        ...
        await self._add_retention_policy_metrics(
            report, retention_service, organization_id
        )

        # Data cleanup metrics
        await self._add_data_cleanup_metrics(
            report, retention_service, period_start, period_end
        )
        ...
        report.summary = {
            ...
            "retention_compliance_score": await self._calculate_retention_compliance_score(
                organization_id
            ),
        }
        ...
        self.reports[report_id] = report
        logger.info(
            f"Generated data retention report {report_id} for organization {organization_id}"
        )
    ...
    async def generate_comprehensive_report(
        ...
        gdpr_report = await self.generate_gdpr_compliance_report(
            organization_id, period_start, period_end
        )
        consent_report = await self.generate_consent_management_report(
            organization_id, period_start, period_end
        )
        retention_report = await self.generate_data_retention_report(
            organization_id, period_start, period_end
        )
        ...
        report.summary = {
            ...
            "high_priority_issues": len(
                [i for i in report.issues if i["severity"] == "high"]
            ),
        ...
        self.reports[report_id] = report
        logger.info(
            f"Generated comprehensive compliance report {report_id} for organization {organization_id}"
        )
    ...
    async def _add_processing_purposes_metrics(
        self, report: ComplianceReport, gdpr_service
    ):
        ...
        consent_based = sum(
            1 for p in purposes.values() if p["lawful_basis"] == "consent"
        )
        report.add_metric(
            ComplianceMetric(
    ...
    async def _add_data_retention_metrics(
        self, report: ComplianceReport, organization_id: str
    ):
    ...
    async def _add_rights_fulfillment_metrics(
        self, report: ComplianceReport, gdpr_service
    ):
    ...
    async def _add_consent_rate_metrics(
        self, report: ComplianceReport, consent_service, organization_id: str
    ):
    ...
    async def _add_consent_withdrawal_metrics(
        self, report: ComplianceReport, consent_service, organization_id: str
    ):
    ...
    async def _add_consent_expiry_metrics(
        self, report: ComplianceReport, consent_service, organization_id: str
    ):
    ...
    async def _add_consent_method_metrics(
        self, report: ComplianceReport, consent_service, organization_id: str
    ):
    ...
    async def _add_retention_policy_metrics(
        self, report: ComplianceReport, retention_service, organization_id: str
    ):
        ...
        report.add_metric(
            ComplianceMetric(
                ...
                status=(
                    "compliant"
                    if retention_report["total_policies"] > 0
                    else "non_compliant"
                ),
    ...
    async def _add_data_cleanup_metrics(
        self,
        report: ComplianceReport,
        retention_service,
        period_start: datetime,
        ...
        report.add_metric(
            ComplianceMetric(
                ...
                status=(
                    "warning"
                    if cleanup_results["total_records_found"] > 0
                    else "compliant"
                ),
    ...
    async def _add_legal_hold_metrics(
        self, report: ComplianceReport, retention_service, organization_id: str
    ):
    ...
    async def _add_storage_metrics(
        self, report: ComplianceReport, organization_id: str
    ):
    ...
    async def _count_data_subjects(self, organization_id: str) -> int:
        ...
        try:
            ...
            user_count = (
                db.query(User).filter(User.organization_id == organization_id).count()
            )
            customer_count = (
                db.query(Customer)
    ...
    async def schedule_automated_reports(
        ...
        logger.info(
            f"Scheduled automated reports for {organization_id}: {[rt.value for rt in report_types]} every {frequency.value}"
        )

...
class DataRetentionService:
    ...
    def __init__(self):
        ...
        self.legal_holds: Dict[str, List[str]] = {}  # organization_id -> list of hold reasons
    ...
    def has_legal_hold(self, organization_id: str) -> bool:
        ...
        return organization_id in self.legal_holds and len(self.legal_holds[organization_id]) > 0
    
    async def get_expired_data(self, policy_name: str, limit: int = 1000) -> List[Dict[str, Any]]:
        ...
        cutoff_date = datetime.now(timezone.utc) - timedelta(days=policy.retention_period.value)
    ...
    async def _get_expired_users(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            users = db.query(User).filter(
                and_(
                    ...
                    User.last_login < cutoff_date - timedelta(days=90)  # Inactive for 90+ days
                )
            ).limit(limit).all()
    ...
    async def _get_expired_customers(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
    ...
    async def _get_expired_reviews(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
    ...
    async def _get_expired_support_tickets(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
    ...
    async def _get_expired_recovery_actions(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
        """Get expired recovery action records"""
        db = next(get_db())
        try:
            actions = db.query(RecoveryAction).filter(
                RecoveryAction.created_at < cutoff_date
            ).limit(limit).all()
    ...
    async def _get_expired_agent_decisions(self, cutoff_date: datetime, limit: int) -> List[Dict[str, Any]]:
    ...
    async def archive_data(self, policy_name: str, data_records: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            archive_path = f"/archives/{policy_name}/{datetime.now().strftime('%Y%m%d')}"
            
            logger.info(f"Archiving {len(data_records)} records for policy {policy_name} to {archive_path}")
    ...
    async def delete_expired_data(self, policy_name: str, data_records: List[Dict[str, Any]]) -> int:
    ...
    async def _delete_users(self, db: Session, records: List[Dict[str, Any]]) -> int:
        ...
        result = db.query(User).filter(User.id.in_(user_ids)).delete(synchronize_session=False)
        return result
    
    async def _delete_customers(self, db: Session, records: List[Dict[str, Any]]) -> int:
        """Delete customer records"""
        customer_ids = [record["id"] for record in records]
        result = db.query(Customer).filter(Customer.id.in_(customer_ids)).delete(synchronize_session=False)
        return result
    ...
    async def _delete_reviews(self, db: Session, records: List[Dict[str, Any]]) -> int:
        ...
        result = db.query(Review).filter(Review.id.in_(review_ids)).delete(synchronize_session=False)
        return result
    
    async def _delete_support_tickets(self, db: Session, records: List[Dict[str, Any]]) -> int:
        """Delete support ticket records"""
        ticket_ids = [record["id"] for record in records]
        result = db.query(SupportTicket).filter(SupportTicket.id.in_(ticket_ids)).delete(synchronize_session=False)
        return result
    
    async def _delete_recovery_actions(self, db: Session, records: List[Dict[str, Any]]) -> int:
        """Delete recovery action records"""
        action_ids = [record["id"] for record in records]
        result = db.query(RecoveryAction).filter(RecoveryAction.id.in_(action_ids)).delete(synchronize_session=False)
        return result
    
    async def _delete_agent_decisions(self, db: Session, records: List[Dict[str, Any]]) -> int:
        """Delete agent decision records"""
        decision_ids = [record["id"] for record in records]
        result = db.query(AgentDecision).filter(AgentDecision.id.in_(decision_ids)).delete(synchronize_session=False)
        return result
            ...
            try:
                ...
                if expired_data:
                    if not dry_run:
                        ...
                        deleted_count = await self.delete_expired_data(policy_name, expired_data)
                ...
                logger.info(f"Processed retention policy {policy_name}: {len(expired_data)} records")
        ...
        return {
            "policies": {
                name: {
                    ...
                    "retention_period_days": policy.retention_period.value if policy.retention_period != RetentionPeriod.PERMANENT else "permanent",
            ...
            "auto_delete_policies": sum(1 for p in self.policies.values() if p.auto_delete)
        }

...
class ErrorTracker:
    ...
    def __init__(self):
        ...
        if SENTRY_AVAILABLE and settings.SENTRY_DSN:
            ...
            logger.warning(
                "Sentry not available or DSN not configured, using fallback error tracking"
            )
...
def capture_exception(
    ...
    error_tracker.capture_exception(
        exception, context, user_id, organization_id, request_id, tags, level
    )
...
def capture_message(
    ...
    error_tracker.capture_message(
        message, level, context, user_id, organization_id, tags
    )
...
def set_user_context(
    user_id: str, organization_id: Optional[str] = None, email: Optional[str] = None
):
    """Set user context for error tracking"""
...
class ErrorTrackingContext:
    ...
    def __enter__(self):
        add_breadcrumb(
            f"Starting operation: {self.operation_name}", category="operation"
        )
        if self.user_id:

...
class TooManyRequestsHTTPException(HTTPException):
    ...
    def __init__(self, detail: str = "Too many requests", headers: Optional[Dict[str, Any]] = None):
...
def convert_exception_to_http(exc: ReviveAIException) -> HTTPException:
    ...
    http_exception_class = EXCEPTION_MAPPING.get(type(exc), InternalServerErrorHTTPException)

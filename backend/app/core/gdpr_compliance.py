...
class GDPRComplianceService:
    ...
    async def _process_access_request(self, request: GDPRRequest):
        ...
        data_subject_data = await self._collect_personal_data(
            request.data_subject_email, request.organization_id
        )
    ...
    async def _process_erasure_request(self, request: GDPRRequest):
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Data erased for {request.data_subject_email}: {deleted_records}"
            )
    ...
    async def _process_portability_request(self, request: GDPRRequest):
        ...
        request.response_data = {
            ...
            "records_exported": sum(
                len(v) if isinstance(v, list) else 1 for v in portable_data.values()
            ),
        }
    ...
    async def _process_objection_request(self, request: GDPRRequest):
        ...
        request.response_data = {
            ...
            "continuing_processing": [
                p for p in processing_purposes if p not in stopped_processing
            ],
        }
    ...
    async def _process_consent_withdrawal(self, request: GDPRRequest):
        ...
        try:
            ...
            logger.info(
                f"Consent withdrawn for {request.data_subject_email}: {consent_types}"
            )
    ...
    async def _collect_personal_data(
        ...
        try:
            ...
            if customer:
                personal_data["customer_profile"] = {
                    ...
                    "last_interaction": (
                        customer.last_interaction.isoformat()
                        if customer.last_interaction
                        else None
                    ),
                    "churn_risk_score": (
                        float(customer.churn_risk_score)
    ...
    async def _collect_portable_data(
        ...
        if "user_account" in all_data:
            portable_data["user_account"] = {
                k: v
                for k, v in all_data["user_account"].items()
        ...
        if "customer_profile" in all_data:
            portable_data["customer_profile"] = {
                k: v
                for k, v in all_data["customer_profile"].items()
            ...
            portable_data["reviews"] = [
                {
                    ...
                    if k in ["platform", "rating", "content", "created_at"]
                }

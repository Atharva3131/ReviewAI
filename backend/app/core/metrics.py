...
class TimeSeries:
    ...
    def add_point(self, value: float, timestamp: Optional[datetime] = None):
        ...
        point = MetricPoint(
            name=self.name, value=value, timestamp=timestamp, tags=self.tags
        )
    ...
    def _aggregate_metrics(self):
        ...
        logger.info(
            ...
            extra={
                ...
                "metric_count": len(self.counters)
                + len(self.gauges)
                + len(self.histograms),
            },
        )
...
class TimingContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            ...
            self.collector.increment_counter(f"{self.name}.count", tags=self.tags)

            # Track errors if exception occurred
            if exc_type:
                error_tags = (self.tags or {}).copy()
                error_tags["error_type"] = exc_type.__name__
                self.collector.increment_counter(f"{self.name}.errors", tags=error_tags)
...
def track_api_request(
    method: str, endpoint: str, status_code: int, response_time: float
):
...
def track_database_operation(
    operation: str, table: str, duration: float, rows_affected: Optional[int] = None
):
...
def track_external_api_call(
    service: str, endpoint: str, status_code: int, response_time: float
):
...
def track_background_task(task_name: str, duration: float, success: bool):
    ...
    tags = {"task_name": task_name, "status": "success" if success else "failure"}
...
def track_user_action(action: str, user_id: str, organization_id: str):
    ...
    tags = {"action": action, "organization_id": organization_id}

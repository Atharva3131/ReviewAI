...
class MonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        # Track request start
        increment_counter(
            "api.requests.started", tags={"method": method, "endpoint": url_path}
        )
        ...
        try:
            ...
            capture_exception(
                ...
                request_id=request_id,
                tags={"endpoint": url_path, "method": method},
            )
    ...
    def _track_request_completion(
        ...
        if response_time > 1.0:  # Slow request threshold
            ...
            # Track slow requests
            increment_counter(
                "api.requests.slow", tags={"method": method, "endpoint": endpoint}
            )
    ...
    def _track_request_error(
        ...
        increment_counter(
            "api.requests.errors",
            tags={"method": method, "endpoint": endpoint, "error_type": error_type},
        )

        # Track error response time
        record_histogram(
            "api.requests.error_duration",
            response_time * 1000,
            tags={"method": method, "endpoint": endpoint, "error_type": error_type},
        )
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if self.request_counts.get(key, 0) > 100:  # More than 100 requests per minute
            ...
            increment_counter(
                "security.rate_limit_exceeded", tags={"client_ip": client_ip}
            )

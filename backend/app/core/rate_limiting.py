...
class AdvancedRateLimiter:
    ...
    async def check_rate_limit(
        ...
        overall_limited = (
            global_result["limited"]
            or endpoint_result["limited"]
            or burst_result["limited"]
        )
    ...
    async def _check_global_limit(
        identifier: str, tier: UserTier, current_time: int, use_sliding_window: bool
    ) -> Dict[str, any]:
        """Check global rate limit"""
    ...
    async def _check_burst_limit(
        identifier: str, tier: UserTier, current_time: int
    ) -> Dict[str, any]:
    ...
    async def _sliding_window_check(
        key: str, limit_config: RateLimit, current_time: int
    ) -> Dict[str, any]:
        """
    ...
    async def _fixed_window_check(
        ...
        window_start = (
            current_time // limit_config.window_seconds
            ...
            if redis_client.redis:
                ...
                if current_count == 1:
                    await redis_client.expire(
                        window_key, limit_config.window_seconds + 1
                    )
            else:
                current_count = 1
    ...
    def get_rate_limit_headers(rate_limit_info: Dict[str, any]) -> Dict[str, str]:
        ...
        headers["X-RateLimit-Policy"] = (
            f"{limit_info.get('limit', 0)};w={limit_info.get('window_seconds', 60)}"
        )

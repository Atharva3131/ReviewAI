...
class CacheManager:
    ...
    async def get_or_set(
        key: str, fetch_func, expire: int = 3600, use_json: bool = True
    ) -> Any:
...
class SessionManager:
    ...
    async def create_session(cls, user_id: str, session_data: dict) -> str:
        ...
        session_data.update({"user_id": user_id, "created_at": int(time.time())})
    ...
    async def update_session(cls, session_id: str, session_data: dict) -> bool:
        ...
        if existing_session:
            ...
            return await redis_client.set_json(
                session_key, existing_session, cls.DEFAULT_EXPIRE
            )
        return False
...
class RateLimiter:
    ...
    async def token_bucket_rate_limit(
        key: str, capacity: int, refill_rate: float, tokens_requested: int = 1
    ) -> tuple[bool, int]:
        """
        ...
        if bucket_data is None:
            # Initialize bucket
            bucket_data = {"tokens": capacity, "last_refill": now}
        else:
            ...
            if tokens_to_add > 0:
                bucket_data["tokens"] = min(
                    capacity, bucket_data["tokens"] + tokens_to_add
                )
...
class AdvancedRateLimiter:
    ...
    async def check_rate_limit(
        ...
        if use_sliding_window:
            is_limited, remaining, reset_time = (
                await RateLimiter.sliding_window_rate_limit(
                    global_key, global_limit["requests"], global_limit["window"]
                )
            )
        else:
        ...
        if endpoint and endpoint in cls.ENDPOINT_LIMITS:
            ...
            if use_sliding_window:
                ep_limited, ep_remaining, ep_reset = (
                    await RateLimiter.sliding_window_rate_limit(
                        endpoint_key,
                ...
                ep_limited, ep_remaining = await RateLimiter.is_rate_limited(
                    endpoint_key, endpoint_limit["requests"], endpoint_limit["window"]
                )
    ...
    async def get_user_tier(
        cls, user_id: str = None, organization_id: str = None
    ) -> str:

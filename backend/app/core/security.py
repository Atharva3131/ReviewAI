...
class SecurityService:
    ...
    def validate_password_strength(password: str) -> Dict[str, Any]:
        ...
        is_valid = (
            checks["length"]
            and checks["max_length"]
            and checks["uppercase"]
            and checks["lowercase"]
            and checks["digit"]
            and checks["not_common"]
            and checks["no_whitespace"]
        )
    ...
    def _has_sequential_chars(password: str, min_length: int = 3) -> bool:
        ...
        for i in range(len(password) - min_length + 1):
            ...
            if substr.isdigit() and all(
                int(substr[j]) == int(substr[0]) + j for j in range(len(substr))
            ):
                return True

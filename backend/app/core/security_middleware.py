...
class CSRFProtectionMiddleware(BaseHTTPMiddleware):
        ...
        if not self._validate_csrf_token(csrf_token, request):
            logger.warning(
                ...
                extra={
                    ...
                    "client_ip": request.client.host if request.client else None,
                    "user_agent": request.headers.get("user-agent"),
                    "csrf_token_sample": csrf_token[:10] + "..." if len(csrf_token) > 10 else csrf_token
                }
            )
            return JSONResponse(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    def _validate_csrf_token(self, token: str, request: Request) -> bool:
        ...
        try:
            ...
            session_data = f"{timestamp}.{user_id}.{request.client.host if request.client else ''}"
...
class InputSanitizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if any(ct in content_type.lower() for ct in ['multipart/form-data', 'application/octet-stream']):
            return await call_next(request)
        ...
        if request.query_params:
            ...
            for key, value in request.query_params.items():
                try:
                    ...
                    logger.warning(f"Invalid query parameter: {key}={value}, error: {e.message}")
                    return JSONResponse(
        ...
        for header_name in headers_to_sanitize:
            ...
            if header_value:
                try:
                    ...
                    logger.warning(f"Failed to sanitize header {header_name}: {header_value}")
...
class AdvancedSecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        permissions_policy = self.config.get('permissions_policy', self._get_default_permissions())
        ...
        if request.url.scheme == "https":
            hsts_header = self.config.get('hsts_header', "max-age=31536000; includeSubDomains; preload")
...
class DDoSProtectionMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, max_requests_per_minute: int = 100, max_requests_per_second: int = 10):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if second_count > self.max_requests_per_second:
            logger.warning(f"DDoS protection: Too many requests per second from {client_ip}")
        ...
        if minute_count > self.max_requests_per_minute:
            logger.warning(f"DDoS protection: Too many requests per minute from {client_ip}")
        ...
        if await self._is_suspicious_request(request, client_ip):
            logger.warning(f"DDoS protection: Suspicious request pattern from {client_ip}")
            return self._create_rate_limit_response("Suspicious request pattern detected")
...
class AuditLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _write_audit_log(self, audit_data: dict):
        ...
        try:
            ...
            await redis_client.set_json(log_key, audit_data, expire=30 * 24 * 60 * 60)  # 30 days
    ...
    async def _log_security_event(self, audit_data: dict, status_code: int):
        ...
        try:
            ...
            await redis_client.set_json(event_key, security_event, expire=90 * 24 * 60 * 60)  # 90 days

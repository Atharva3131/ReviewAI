...
class SQLSecurityValidator:
    ...
    DANGEROUS_KEYWORDS = {
        ...
        'xp_cmdshell', 'sp_executesql', 'sp_configure', 'sp_adduser',
        'OPENROWSET', 'OPENDATASOURCE', 'BULK', 'LOAD_FILE', 'INTO OUTFILE'
    }
    ...
    @classmethod
    def validate_query_string(cls, query: str, allow_keywords: List[str] = None) -> bool:
        """
    ...
    def validate_user_input(cls, user_input: str, context: str = "general") -> str:
        ...
        for pattern in cls.INJECTION_PATTERNS:
            if re.search(pattern, user_input, re.IGNORECASE):
                ...
                raise SQLInjectionError(
                    f"Invalid input detected in {context}",
                    query=user_input
                )
...
class SafeQueryBuilder:
    ...
    def build(self) -> str:
        ...
        SQLSecurityValidator.validate_query_string(query, allow_keywords=["SELECT", "FROM", "WHERE", "ORDER", "LIMIT"])
...
class DatabaseSecurityAuditor:
    ...
    def _check_dangerous_operations(self, parsed_query, audit_result):
        ...
        if any(op in query_str for op in ['DELETE', 'UPDATE']) and 'WHERE' not in query_str:
            audit_result["security_issues"].append("Data modification without WHERE clause")
            audit_result["recommendations"].append("Always use WHERE clause with DELETE/UPDATE")
        
        # Check for SELECT *
        if 'SELECT *' in query_str:
            audit_result["security_issues"].append("SELECT * usage detected")
            audit_result["recommendations"].append("Specify explicit column names instead of SELECT *")
        
        # Check for administrative functions
        admin_functions = ['xp_', 'sp_', 'EXEC', 'EXECUTE']
        for func in admin_functions:
            if func in query_str:
                audit_result["security_issues"].append(f"Administrative function detected: {func}")
                audit_result["recommendations"].append("Avoid administrative functions in application queries")
    ...
    def _check_parameterization(self, query: str, parameters: Dict, audit_result):
        ...
        for pattern in concat_patterns:
            if re.search(pattern, query):
                audit_result["security_issues"].append("String concatenation detected in query")
                audit_result["recommendations"].append("Use parameterized queries instead of string concatenation")
                break
        
        # Check if parameters are used
        if ':' in query and not parameters:
            audit_result["security_issues"].append("Query has parameter placeholders but no parameters provided")
        ...
        for pattern in literal_patterns:
            if re.search(pattern, query):
                audit_result["security_issues"].append("Literal values detected - consider parameterization")
                audit_result["recommendations"].append("Use parameters for dynamic values")
                break
    ...
    def _check_information_disclosure(self, parsed_query, audit_result):
        ...
        for table in system_tables:
            if table in query_str:
                audit_result["security_issues"].append(f"System table access detected: {table}")
                audit_result["recommendations"].append("Avoid querying system tables from application code")
        
        # Check for error-prone functions
        error_functions = ['CONVERT', 'CAST', 'EXTRACTVALUE']
        for func in error_functions:
            if func in query_str:
                audit_result["security_issues"].append(f"Error-prone function detected: {func}")
                audit_result["recommendations"].append("Handle conversion errors properly")
    ...
    def _calculate_risk_level(self, audit_result):
        ...
        high_risk_keywords = ['administrative function', 'system table', 'without WHERE clause']
        has_high_risk = any(

...
class InputSanitizer:
    ...
    ALLOWED_HTML_TAGS = ["p", "br", "strong", "em", "u", "ol", "ul", "li", "a", "span"]
    ...
    def sanitize_string(
        ...
        if not isinstance(value, str):
            raise ValidationError(
                "Input must be a string", details={"type": type(value).__name__}
            )
    ...
    def sanitize_json_field(cls, value: Any, field_name: str) -> Any:
        ...
        if isinstance(value, str):
            ...
            return {
                cls.sanitize_string(k, max_length=100): cls.sanitize_json_field(
                    v, f"{field_name}.{k}"
                )
            ...
            return [
                cls.sanitize_json_field(item, f"{field_name}[{i}]")
...
def sanitize_string_validator(max_length: int = None, allow_html: bool = False):
    ...
    def validator_func(cls, v):
        ...
        return InputSanitizer.sanitize_string(
            v, max_length=max_length, allow_html=allow_html
        )

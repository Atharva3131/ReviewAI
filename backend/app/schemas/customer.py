...
class CustomerBase(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Customer name"
    )
    context_summary: Optional[str] = Field(
        None, max_length=2000, description="Customer context summary"
    )
...
class CustomerResponse(CustomerBase, UUIDMixin, TimestampMixin, OrganizationMixin):
    ...
    churn_risk_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Churn risk score"
    )
    bad_review_likelihood: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Bad review likelihood"
    )
...
class CustomerListFilter(FilterParams):
    ...
    risk_level: Optional[str] = Field(
        None, pattern="^(low|medium|high)$", description="Risk level filter"
    )
    ...
    min_churn_risk: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Minimum churn risk"
    )
    max_churn_risk: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Maximum churn risk"
    )
...
class SupportTicketBase(BaseModel):
    ...
    external_id: Optional[str] = Field(
        None, max_length=255, description="External ticket ID"
    )
    subject: str = Field(
...
class SupportTicketUpdate(BaseModel):
    ...
    subject: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Ticket subject"
    )
...
class SupportTicketResponse(
    SupportTicketBase, UUIDMixin, TimestampMixin, OrganizationMixin
):
    """Schema for support ticket response"""

    customer_id: uuid.UUID = Field(..., description="Customer ID")
    sentiment_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Sentiment score"
    )
...
class RecoveryActionResponse(
    RecoveryActionBase, UUIDMixin, TimestampMixin, OrganizationMixin
):
...
class CustomerRecoveryRequest(BaseModel):
    ...
    def validate_trigger_type(cls, v):
        ...
        if v not in allowed_triggers:
            raise ValueError(
                f'trigger_type must be one of: {", ".join(allowed_triggers)}'
            )
        return v
...
class RecoveryPlan(BaseModel):
    ...
    bad_review_likelihood: float = Field(
        ..., ge=0.0, le=1.0, description="Bad review likelihood"
    )
    actions: List[RecoveryActionResponse] = Field(
        [], description="Recommended recovery actions"
    )
    reasoning: Optional[str] = Field(None, description="Recovery plan reasoning")
    confidence_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Plan confidence"
    )
...
class RiskFactors(BaseModel):
    ...
    avg_sentiment: float = Field(
        0.5, ge=0.0, le=1.0, description="Average sentiment score"
    )
    escalation_count: int = Field(0, ge=0, description="Number of escalations")
    days_since_last_interaction: int = Field(
        0, ge=0, description="Days since last interaction"
    )
    negative_review_count: int = Field(
        0, ge=0, description="Number of negative reviews"
    )
    response_time_avg: Optional[float] = Field(
        None, ge=0.0, description="Average response time in hours"
    )
...
class BulkCustomerUpdate(BaseModel):
    ...
    customer_ids: List[uuid.UUID] = Field(
        ..., min_items=1, max_items=100, description="Customer IDs"
    )
...
class BulkRecoveryRequest(BaseModel):
    ...
    customer_ids: List[uuid.UUID] = Field(
        ..., min_items=1, max_items=50, description="Customer IDs"
    )
    trigger_type: str = Field(..., description="Recovery trigger type")
    context: Optional[Dict[str, Any]] = Field(None, description="Recovery context")

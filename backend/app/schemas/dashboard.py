...
class KPIData(BaseModel):
    ...
    recovery_success_rate: KPIValue = Field(
        ..., description="Recovery action success rate percentage"
    )
...
class ActivityItem(BaseModel):
    ...
    type: str = Field(
        ..., description="Activity type: review, recovery_action, agent_decision"
    )
...
class ChartData(BaseModel):
    ...
    rating_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Rating distribution by star rating (1-5)"
    )
    issue_categories: Dict[str, int] = Field(
        default_factory=dict, description="Issue category distribution"
    )
...
class TrendData(BaseModel):
    ...
    sentiment_over_time: List[TrendDataPoint] = Field(
        default_factory=list, description="Sentiment trends over time"
    )
    review_volume_over_time: List[TrendDataPoint] = Field(
        default_factory=list, description="Review volume trends over time"
    )
    recovery_actions_over_time: List[TrendDataPoint] = Field(
        default_factory=list, description="Recovery action trends over time"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "charts": {
                    ...
                    "rating_distribution": {"1": 5, "2": 10, "3": 15, "4": 35, "5": 35},
                    "issue_categories": {
                        "support": 45,
                        "quality": 32,
                        "delivery": 28,
...
class AverageRatingResponse(BaseModel):
    ...
    previous_total_reviews: int = Field(
        ..., description="Previous period total reviews"
    )
...
class AtRiskCustomersResponse(BaseModel):
    ...
    high_review_risk: int = Field(
        ..., description="Customers with high bad review likelihood"
    )

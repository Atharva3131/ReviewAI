...
class AgentDecisionService:
        ...
        if filters:
            ...
            if filters.decision_type:
                try:
                    ...
                    query = query.where(
                        AgentDecision.decision_type == decision_type_enum
                    )
                except ValueError:
            ...
            if filters.confidence_min is not None:
                query = query.where(
                    AgentDecision.confidence_score >= filters.confidence_min
                )

            if filters.confidence_max is not None:
                query = query.where(
                    AgentDecision.confidence_score <= filters.confidence_max
                )
    ...
    async def get_decision_by_id(
        db: AsyncSession, decision_id: str, organization_id: str
    ...
    async def get_decision_stats(
        ...
        decisions_by_type = {
            decision_type.value: count for decision_type, count in type_dist_result
        }
        ...
        avg_confidence_result = await db.execute(
            select(func.avg(AgentDecision.confidence_score)).where(
                AgentDecision.organization_id == organization_id
            )
        )
        ...
        approval_rate = (
            (approved_count / reviewed_count * 100) if reviewed_count > 0 else 0.0
        )
        ...
        avg_processing_result = await db.execute(
            select(func.avg(AgentDecision.processing_time_ms)).where(
                and_(
                    AgentDecision.organization_id == organization_id,
    ...
    async def mark_decision_failed(
        db: AsyncSession, decision_id: str, organization_id: str, error_message: str
    ) -> AgentDecision:
    ...
    async def get_decision_performance_metrics(
        ...
        outcome_result = await db.execute(
            select(
                ...
                func.sum(func.cast(AgentDecision.outcome_success, db.Integer)).label(
                    "successful"
                ),
        ...
        return {
            ...
            "success_rate": (
                (outcome_stats.successful / outcome_stats.total * 100)
            ...
            "escalation_rate": (
                (escalation_count / total_decisions * 100) if total_decisions else 0
            ),
        }

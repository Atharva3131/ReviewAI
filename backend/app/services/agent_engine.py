...
class AgentEngine:
    ...
    async def process_review(
        ...
        try:
            ...
            sentiment_result = await self.sentiment_service.analyze_sentiment(
                review.content or "", review.rating
            )
            ...
            categorization_result = await self.categorization_service.categorize_issues(
                review.content or "", review.title, review.rating
            )
            ...
            logger.info(
                f"Review decision completed: {review.id} -> {result.decision_type}"
            )
            return result
    ...
    async def process_support_ticket(
        ...
        try:
            ...
            sentiment_result = await self.sentiment_service.analyze_sentiment(
                ticket.content or "", None  # No rating for tickets
            )
            ...
            # Step 3: Categorize issues
            categorization_result = await self.categorization_service.categorize_issues(
                ticket.content or "", ticket.subject, None  # No rating
            )
            ...
            logger.info(
                f"Ticket decision completed: {ticket.id} -> {result.decision_type}"
            )
            return result
    ...
    async def validate_decision(
        self, decision_result: AgentDecisionResult, input_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            input_data.get("rating", 5) <= 2
            and decision_result.decision_type != DecisionType.ESCALATE
        ):
    ...
    async def _validate_generated_content(self, content: str) -> Dict[str, Any]:
        ...
        for phrase in problematic_phrases:
            if phrase in content_lower:
                concerns.append(
                    f"Content contains potentially problematic phrase: '{phrase}'"
                )

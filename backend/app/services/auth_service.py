...
class AuthService:
    ...
    async def register_user(
        db: AsyncSession, registration_data: UserRegistration
    ...
    @staticmethod
    async def reset_password(db: AsyncSession, email: str) -> str:
        ...
        await redis_client.set(
            f"password_reset:{reset_token}", str(user.id), expire=60 * 60  # 1 hour
        )

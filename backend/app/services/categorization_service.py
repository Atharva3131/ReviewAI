...
class CategorizationService:
    ...
    CATEGORY_RELATIONSHIPS = {
        ...
        "conflicting": {
            (
                IssueCategory.PRICING,
                IssueCategory.QUALITY,
            ): 0.3,  # Can conflict but not always
        },
    }

    @staticmethod
    async def categorize_issues(
        content: str, title: str = None, rating: int = None, min_confidence: float = 0.3
    ) -> Dict[str, Any]:
        """
        ...
        adjusted_scores = CategorizationService._apply_category_relationships(
            category_scores
        )
        ...
        sorted_categories = sorted(
            confident_categories.items(), key=lambda x: x[1], reverse=True
        )
        ...
        return CategorizationService._create_result(
            categories, confidences, primary_category, adjusted_scores, keyword_evidence
        )

    @staticmethod
    def _calculate_category_score(
        content: str, category: IssueCategory, rating: int = None
        ...
        if rating is not None:
            rating_adjustment = CategorizationService._get_rating_adjustment(
                category, rating
            )
        ...
        for (cat1, cat2), boost_factor in complementary.items():
            ...
            if cat1_val in scores and cat2_val in scores:
                if scores[cat1_val] > 0.3 and scores[cat2_val] > 0.3:
                    ...
                    adjusted_scores[cat1_val] = min(
                        scores[cat1_val] * boost_factor, 1.0
                    )
                    adjusted_scores[cat2_val] = min(
                        scores[cat2_val] * boost_factor, 1.0
                    )
    ...
    async def validate_categorization(
        content: str, predicted_categories: List[IssueCategory], rating: int = None
    ) -> Dict[str, Any]:
        ...
        high_score_categories = [
            cat for cat, score in analysis["category_scores"].items() if score > 0.6
        ]
        ...
        for high_score_cat in high_score_categories:
            if high_score_cat not in predicted_category_values:
                validation_issues.append(
                    f"High-confidence category '{high_score_cat}' not predicted"
                )
        ...
        for cat in predicted_categories:
            if analysis["category_scores"].get(cat.value, 0) < 0.3:
                validation_issues.append(
                    f"Low confidence for predicted category '{cat.value}'"
                )
        ...
        return {
            ...
            "confidence_summary": {
                "avg_confidence": (
                    sum(analysis["confidences"].values()) / len(analysis["confidences"])
                    if analysis["confidences"]
                    else 0
                ),
                "min_confidence": (
                    min(analysis["confidences"].values())
                    if analysis["confidences"]
                    else 0
                ),
                "max_confidence": (
                    max(analysis["confidences"].values())
                    if analysis["confidences"]
                    else 0
                ),
            },
    ...
    async def get_category_suggestions(
        content: str, current_categories: List[IssueCategory] = None
    ) -> Dict[str, Any]:
        """Get suggestions for additional categories"""

        analysis = await CategorizationService.categorize_issues(
                ...
                suggestions.append(
                    {
                        ...
                        "evidence": analysis["keyword_evidence"].get(cat, [])[
                            :3
                        ],  # Top 3 keywords
                    }
                )

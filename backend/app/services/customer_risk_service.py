...
class CustomerRiskAssessmentService:
    ...
    async def _calculate_churn_risk(
        self, customer: Customer, db: AsyncSession
    ) -> float:
        ...
        churn_risk = sum(
            factors[factor] * self.churn_weights[factor] for factor in factors
        )
    ...
    async def _calculate_bad_review_likelihood(
        ...
        factors["recent_sentiment"] = await self._calculate_recent_sentiment_risk(
            customer, db
        )

        # Factor 2: Active support tickets
        factors["support_tickets"] = await self._calculate_active_support_risk(
            customer, db
        )

        # Factor 3: Escalation history
        factors["escalation_history"] = await self._calculate_escalation_history_risk(
            customer, db
        )

        # Factor 4: Response time satisfaction
        factors["response_time"] = await self._calculate_response_time_risk(
            customer, db
        )

        # Factor 5: Resolution rate
        factors["resolution_rate"] = await self._calculate_resolution_rate_risk(
            customer, db
        )

        # Calculate weighted score
        review_risk = sum(
            factors[factor] * self.review_weights[factor] for factor in factors
        )
    ...
    async def _calculate_sentiment_risk(
        ...
        avg_sentiment = sum(float(score) for score in sentiment_scores) / len(
            sentiment_scores
        )
    ...
    async def _get_risk_factors(
        ...
        return {
            ...
            "avg_rating_given": (
                float(customer.avg_rating_given) if customer.avg_rating_given else None
            ),
            "interaction_count": customer.interaction_count,
            "is_high_value": customer.is_high_value,
            "lifetime_value": (
                float(customer.lifetime_value) if customer.lifetime_value else None
            ),
        }
    ...
    async def update_customer_risk_scores(
        ...
        if customer:
            customer.update_risk_scores(
                risk_assessment["churn_risk"], risk_assessment["bad_review_likelihood"]
            )
            await db.commit()
    ...
    async def get_at_risk_customers(
        ...
        stmt = (
            ...
            .order_by(
                Customer.churn_risk_score.desc(), Customer.bad_review_likelihood.desc()
            )
            .limit(limit)
        )
    ...
    async def batch_update_risk_scores(
        self, organization_id: str, db: AsyncSession, limit: int = 50
    ) -> Dict[str, int]:

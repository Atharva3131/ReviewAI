        ...
        for attempt in range(self.retry_config.max_retries + 1):
            try:
                ...
                if response.status_code < 400:
                    ...
                    await self._log_request(method, url, response.status_code, response_time, True)
                ...
                # Handle client errors (4xx) - don't retry
                    ...
                    await self._log_request(method, url, response.status_code, response_time, False)
                ...
                if attempt < self.retry_config.max_retries:
                    delay = self._calculate_retry_delay(attempt)
                    logger.warning(f"Timeout from {self.service_name}, retrying in {delay}s")
                    await asyncio.sleep(delay)
                    continue
                ...
                if attempt < self.retry_config.max_retries:
                    delay = self._calculate_retry_delay(attempt)
                    logger.error(f"Error from {self.service_name}: {e}, retrying in {delay}s")
                    await asyncio.sleep(delay)
                    continue

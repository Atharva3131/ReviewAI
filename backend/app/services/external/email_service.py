...
class SendGridEmailService(BaseExternalService):
    ...
    async def send_email(self, message: EmailMessage) -> ServiceResponse:
        ...
        try:
            ...
            if response.success:
                logger.info(f"Email sent successfully via SendGrid to {len(message.to)} recipients")
                return ServiceResponse(
                    success=True,
                    data={
    ...
    async def send_template_email(
        ...
        try:
            message = EmailMessage(
                to=to_emails,
                subject=subject or "{{subject}}",  # Use template subject if not provided
...
class AWSEmailService(BaseExternalService):
    ...
    async def test_connection(self) -> ServiceResponse:
        ...
        try:
            ...
            response = await self._make_request("POST", "/", data={
                "Action": "GetSendQuota",
                "Version": "2010-12-01"
            })
    ...
    async def send_email(self, message: EmailMessage) -> ServiceResponse:
        ...
        try:
            ...
            if response.success:
                logger.info(f"Email sent successfully via AWS SES to {len(message.to)} recipients")
                return ServiceResponse(
                    success=True,
                    data={
...
class MockEmailService(BaseExternalService):
    ...
    async def test_connection(self) -> ServiceResponse:
        ...
        return ServiceResponse(
            success=True,
            data={"message": "Mock email service connection successful"}
        )
...
class EmailServiceManager:
    ...
    def __init__(self, primary_provider: EmailProvider = EmailProvider.SENDGRID):
        ...
        self.fallback_order = [EmailProvider.SENDGRID, EmailProvider.AWS_SES, EmailProvider.MOCK]
    ...
    async def send_email(self, message: EmailMessage, provider: Optional[EmailProvider] = None) -> ServiceResponse:
        ...
        for provider_type in providers_to_try:
            ...
            try:
                ...
                if response.success:
                    ...
                    logger.warning(f"Email failed via {provider_type.value}: {response.error}")
    ...
    async def send_review_response_notification(
        ...
        message = EmailMessage(
            ...
            custom_args={"email_type": "review_notification", "review_id": str(review_details.get('id', ''))}
        )
...
def create_email_service_manager(primary_provider: EmailProvider = EmailProvider.SENDGRID) -> EmailServiceManager:

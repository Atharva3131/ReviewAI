...
class ExternalServiceErrorHandler:
    ...
    def _categorize_error(self, error: Exception, status_code: Optional[int]) -> ErrorCategory:
    ...
    def _determine_severity(self, category: ErrorCategory, status_code: Optional[int]) -> ErrorSeverity:
    ...
    async def _log_error(self, error: ServiceError):
        ...
        if error.severity == ErrorSeverity.CRITICAL:
            logger.critical(f"Critical error in {error.context.service_name}: {error.message}", extra=log_data)
        elif error.severity == ErrorSeverity.HIGH:
            logger.error(f"High severity error in {error.context.service_name}: {error.message}", extra=log_data)
        elif error.severity == ErrorSeverity.MEDIUM:
            logger.warning(f"Medium severity error in {error.context.service_name}: {error.message}", extra=log_data)
        else:
            logger.info(f"Low severity error in {error.context.service_name}: {error.message}", extra=log_data)
    
    async def get_service_errors(self, service_name: str, limit: int = 50) -> List[Dict[str, Any]]:
...
async def handle_authentication_error(error: ServiceError):
    ...
    logger.warning(f"Authentication error in {error.context.service_name}, may need to refresh tokens")
...
async def handle_rate_limit_error(error: ServiceError):
    ...
    logger.info(f"Rate limit hit for {error.context.service_name}, implementing backoff")
...
error_handler.register_error_callback(ErrorCategory.AUTHENTICATION, handle_authentication_error)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.info(f"Retrying {operation} for {service_name} in {delay}s (attempt {attempt + 1})")

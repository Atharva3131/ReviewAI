...
class GoogleReviewsService(BaseExternalService):
        ...
        try:
            ...
            if response.success:
                ...
                return ServiceResponse(
                    success=True,
                    data={"message": "Connection successful", "accounts": response.data}
                )
            else:
    ...
    async def get_locations(self, account_id: str) -> ServiceResponse:
        ...
        try:
            ...
            if response.success and response.data:
                ...
                self.location_ids = [loc.get("name", "").split("/")[-1] for loc in locations]
    ...
    async def get_reviews(
        ...
        try:
            ...
            if response.success and response.data:
                ...
                logger.info(f"Retrieved {len(reviews)} reviews from Google location {location_id}")
                return ServiceResponse(
                    success=True,
                    data={
    ...
    async def get_all_reviews(self, since_date: Optional[datetime] = None) -> ServiceResponse:
        ...
        try:
            ...
            for location_id in self.location_ids:
                try:
                    ...
                    if response.success and response.data:
                        ...
                        if since_date:
                            location_reviews = [
                                review for review in location_reviews
                                if datetime.fromisoformat(review["created_at"].replace("Z", "+00:00")) >= since_date
                            ]
            ...
            logger.info(f"Retrieved {len(all_reviews)} total reviews from {len(self.location_ids)} locations")
    ...
    def _parse_google_review(self, review_data: Dict[str, Any], location_id: str) -> GoogleReview:
...
class GoogleReviewsWebhookSimulator:
    ...
    async def simulate_webhook_polling(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.success and response.data:
                ...
                logger.info(f"Simulated {len(webhook_events)} Google review webhook events")
    ...
    async def generate_test_review_event(self, rating: int = 5, content: str = None) -> Dict[str, Any]:
        ...
        test_review = {
            ...
            "rating": rating,
            "content": content or f"This is a test {rating}-star review for development purposes.",
...
def create_webhook_simulator(google_service: GoogleReviewsService) -> GoogleReviewsWebhookSimulator:

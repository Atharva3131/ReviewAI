...
class WhatsAppBusinessService(BaseExternalService):
    ...
    async def send_text_message(
        self, to: str, message: str, context: Optional[ErrorContext] = None
        ...
        async def _send():
            ...
            if response.success:
                ...
                whatsapp_message = WhatsAppMessage(
                    message_id=message_data.get("messages", [{}])[0].get(
                        "id", str(uuid.uuid4())
                    ),
                    to=to,
    ...
    async def send_template_message(
        ...
        async def _send():
            # Mock implementation for development
            if settings.ENVIRONMENT == "development":
                return await self._mock_send_template(
                    to, template_name, template_params
                )
    ...
    async def get_message_status(
        self, message_id: str, context: Optional[ErrorContext] = None
    ...
    async def _mock_send_message(
        self, to: str, message: str, message_type: MessageType
    ) -> ServiceResponse[WhatsAppMessage]:
        ...
        if await self._should_simulate_failure(0.05):  # 5% failure rate
            return ServiceResponse(
                success=False, error="Mock WhatsApp API error: Rate limit exceeded"
            )
    ...
    async def _mock_send_template(
        self, to: str, template_name: str, template_params: List[str]
    ) -> ServiceResponse[WhatsAppMessage]:
        ...
        if await self._should_simulate_failure(0.03):  # 3% failure rate
            return ServiceResponse(
                success=False, error="Mock WhatsApp API error: Template not approved"
            )

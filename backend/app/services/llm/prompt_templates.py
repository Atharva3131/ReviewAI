...
- Keep it concise (under 150 words)""",
            variables=[
                TemplateVariable("rating", "number", True, description="Review rating 1-5"),
                TemplateVariable("review_content", "string", True, description="The review text"),
                TemplateVariable("customer_name", "string", False, "Valued Customer", "Customer's name"),
                TemplateVariable("business_type", "string", False, "business", "Type of business")
            ],
...
- Include a gesture of goodwill if appropriate""",
            variables=[
                TemplateVariable("customer_name", "string", True, description="Customer's name"),
                TemplateVariable("issue_type", "string", True, description="Type of issue (service, product, billing, etc.)"),
                TemplateVariable("concerns", "string", True, description="Specific customer concerns"),
                TemplateVariable("customer_value", "string", False, "valued", "Customer value tier"),
                TemplateVariable("solution", "string", True, description="Proposed solution"),
                TemplateVariable("contact_info", "string", True, description="Contact information")
            ],
...
- Maintain a sincere and professional tone""",
            variables=[
                TemplateVariable("customer_name", "string", True, description="Customer's name"),
                TemplateVariable("issue_description", "string", True, description="What went wrong"),
                TemplateVariable("impact", "string", True, description="How it affected the customer"),
                TemplateVariable("our_fault", "string", True, description="How we were responsible"),
                TemplateVariable("corrective_actions", "string", True, description="Steps to fix the issue"),
                TemplateVariable("prevention", "string", False, "improved processes", "Prevention measures")
            ],
...
- Include a clear call to action""",
            variables=[
                TemplateVariable("customer_name", "string", True, description="Customer's name"),
                TemplateVariable("discount_percentage", "number", True, description="Discount percentage"),
                TemplateVariable("discount_code", "string", True, description="Discount code"),
                TemplateVariable("expiry_date", "string", True, description="Offer expiry date"),
                TemplateVariable("min_purchase", "string", False, "No minimum", "Minimum purchase requirement"),
                TemplateVariable("offer_reason", "string", False, "loyalty", "Reason for the offer"),
                TemplateVariable("focus_area", "string", False, "any purchase", "Product/service focus")
            ],
    ...
    def select_template_for_ab_test(
        ...
        test_templates = [
            ...
            if (template.type == template_type and 
                template.ab_test_group == ab_test_group and
                template.status == TemplateStatus.TESTING)
        ]
    ...
    def update_template_metrics(
        ...
        if template.success_rate is None:
            ...
            template.success_rate = (
                (template.success_rate * (total_uses - 1) + (1.0 if success else 0.0)) / total_uses
            )
        ...
        if template.avg_response_time is None:
            ...
            template.avg_response_time = (
                (template.avg_response_time * (total_uses - 1) + response_time) / total_uses
            )
        ...
        if customer_satisfaction is not None:
            if template.customer_satisfaction is None:
                ...
                template.customer_satisfaction = (
                    (template.customer_satisfaction * (total_uses - 1) + customer_satisfaction) / total_uses
                )

...
from .base_provider import LLMRequest, LLMResponse, LLMMessage, LLMModel, LLMError
from .provider_factory import get_default_llm_provider, get_llm_provider, LLMProvider
from .prompt_templates import get_template_manager, TemplateType, TemplateManager
...
class ContentSanitizer:
    ...
    def validate_content(self, content: str, content_type: str = 'general') -> Dict[str, Any]:
        ...
        if len(content) > 2000:  # Configurable limit
            ...
            validation_results['recommendations'].append('Consider shortening the message')
        
        # Content type specific validation
        if content_type == 'email':
            if 'subject:' not in content.lower():
                validation_results['recommendations'].append('Consider adding a subject line')
        
        elif content_type == 'review_response':
            if 'thank' not in content.lower():
                validation_results['recommendations'].append('Consider thanking the customer')
        ...
        politeness_score = sum(1 for word in polite_words if word in content.lower()) / len(polite_words)
        quality_factors.append(min(politeness_score * 2, 1.0))
        
        # Professional tone
        professional_indicators = ['sincerely', 'regards', 'best', 'respectfully']
        professional_score = sum(1 for word in professional_indicators if word in content.lower()) / len(professional_indicators)
...
class LLMResponseGenerationService:
    ...
    async def generate_response(self, request: GenerationRequest) -> GenerationResponse:
        ...
        try:
            ...
            # Render template
            rendered = self.template_manager.render_template(template.id, request.variables)
            ...
            if isinstance(e, LLMError):
                ...
                raise LLMError(f"Response generation failed: {str(e)}", LLMProvider.MOCK)
    ...
    def _select_template(self, template_type: TemplateType):
        ...
        active_templates = [t for t in templates if t.status.value in ['active', 'testing']]
    ...
    def _check_rate_limits(self):
        ...
        if len(recent_requests) >= self.rate_limits['requests_per_minute']:
            raise LLMError("Rate limit exceeded: too many requests per minute", LLMProvider.MOCK)
        
        if len(self.request_history) >= self.rate_limits['requests_per_hour']:
            raise LLMError("Rate limit exceeded: too many requests per hour", LLMProvider.MOCK)

...
class MetricsCacheService:
                    ...
                    if warm_time > 0:
                        self.warming_tasks[cache_key] = asyncio.create_task(
                            self._schedule_cache_warming(
                                cache_key, fetch_func, warm_time
                            )
                        )
    ...
    async def _schedule_cache_warming(
        self, cache_key: str, fetch_func: Callable, delay: int
    ):
        """Schedule cache warming task"""
        try:
            await asyncio.sleep(delay)
            await self.warm_cache(
                cache_key, fetch_func, self.CACHE_TTLS.get("kpis", 300)
            )
...
def cached_metrics(
    cache_type: str = "kpis", key_suffix: str = "", warm_cache: bool = True
):
    """
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            cache_key = CacheManager.make_key(
                cache_type, func_name, organization_id, key_suffix, prefix="revive_ai"
            )
...
class MetricsAggregationCache:
    ...
    async def get_daily_aggregates(
        organization_id: str, date: datetime
    ) -> Optional[Dict[str, Any]]:
        """Get daily aggregated metrics"""
        date_str = date.strftime("%Y-%m-%d")
        cache_key = CacheManager.make_key(
            "daily_aggregates", organization_id, date_str, prefix="revive_ai"
        )
    ...
    async def set_daily_aggregates(
        organization_id: str, date: datetime, aggregates: Dict[str, Any]
    ):
    ...
    async def get_hourly_aggregates(
        organization_id: str, hour: datetime
    ) -> Optional[Dict[str, Any]]:
        """Get hourly aggregated metrics"""
        hour_str = hour.strftime("%Y-%m-%d-%H")
        cache_key = CacheManager.make_key(
            "hourly_aggregates", organization_id, hour_str, prefix="revive_ai"
        )
    ...
    async def set_hourly_aggregates(
        organization_id: str, hour: datetime, aggregates: Dict[str, Any]
    ):
                ...
                for time_range in ["7d", "30d", "90d"]:
                    ...
                    if redis_client.redis:
                        ...
                        if ttl < 60:  # Less than 60 seconds remaining
                            await service.get_comprehensive_metrics(

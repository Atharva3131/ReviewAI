...
class MetricsWebSocketManager:
    ...
    async def _periodic_updates(self, organization_id: str):
        ...
        try:
            ...
            logger.info(
                f"Periodic updates cancelled for organization {organization_id}"
            )
        except Exception as e:
            logger.error(
                f"Error in periodic updates for organization {organization_id}: {e}"
            )

    async def _get_lightweight_metrics(
        ...
        try:
            ...
            cache_key = CacheManager.make_key(
                "realtime_metrics", organization_id, prefix="revive_ai"
            )
...
class RealTimeMetricsService:
        ...
        try:
            ...
            async for db in get_async_db():
                ...
                metrics = await dashboard_service.get_comprehensive_metrics(
                    organization_id=organization_id, time_range="30d"
                )
    ...
    async def on_review_created(organization_id: str, review_data: Dict[str, Any]):
        ...
        try:
            ...
            if (
                review_data.get("rating", 5) <= 2
                and review_data.get("urgency_level") == "high"
            ):
                alert = {
    ...
    @staticmethod
    async def on_recovery_action_completed(
        organization_id: str, action_data: Dict[str, Any]
    ):
    ...
    @staticmethod
    async def on_customer_risk_updated(
        organization_id: str, customer_data: Dict[str, Any]
    ):

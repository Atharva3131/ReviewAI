...
class RecoveryMetricsService:
    ...
    async def get_recovery_success_metrics(
        self, organization_id: str, db: AsyncSession, period: str = "monthly"
    ) -> Dict[str, Any]:
        ...
        customer_metrics = await self._get_customer_outcome_metrics(
            organization_id, db, start_date
        )
        ...
        effectiveness_metrics = await self._get_action_effectiveness_metrics(
            organization_id, db, start_date
        )
    ...
    async def _get_action_metrics(
        self, organization_id: str, db: AsyncSession, start_date: datetime
    ) -> Dict[str, Any]:
        """Get basic recovery action metrics"""
        ...
        status_counts = await db.execute(
            select(RecoveryAction.status, func.count(RecoveryAction.id))
            .where(
                and_(
                    RecoveryAction.organization_id == organization_id,
                    RecoveryAction.created_at >= start_date,
        ...
        success_rate = (
            (successful_count / executed_count) if executed_count > 0 else 0.0
        )
    ...
    async def _get_customer_outcome_metrics(
        ...
        customers_with_actions = await db.execute(
            select(func.count(func.distinct(RecoveryAction.customer_id))).where(
                and_(
                    RecoveryAction.organization_id == organization_id,
                    RecoveryAction.created_at >= start_date,
                )
        ...
        response_rate = (
            (responded_count / customers_targeted) if customers_targeted > 0 else 0.0
        )

        # Average outcome rating for responded customers
        avg_outcome = await db.execute(
            select(func.avg(RecoveryAction.outcome_rating)).where(
                and_(
                    RecoveryAction.organization_id == organization_id,
                    RecoveryAction.created_at >= start_date,
        ...
        estimated_churn_prevented = int(
            responded_count * 0.7
        )  # Estimate 70% of responders retained
    ...
    async def _get_roi_metrics(
        ...
        avg_customer_value = await db.execute(
            select(func.avg(Customer.lifetime_value)).where(
                and_(
        ...
        estimated_revenue_retained = (
            retained_count * avg_ltv * 0.5
        )  # Assume 50% of LTV retained

        # Calculate ROI
        roi = (
            ((estimated_revenue_retained - total_cost) / total_cost)
            if total_cost > 0
            else 0.0
        )
    ...
    async def _get_action_effectiveness_metrics(
        ...
        effectiveness_query = await db.execute(
            select(
                ...
                func.sum(
                    case((RecoveryAction.customer_responded == True, 1), else_=0)
                ).label("responded"),
    ...
    async def get_customer_recovery_history(
        ...
        response_rate = (
            (responded_actions / total_actions) if total_actions > 0 else 0.0
        )
        ...
        avg_outcome = (
            sum(outcome_ratings) / len(outcome_ratings) if outcome_ratings else 0.0
        )

...
from app.models.recovery_action import ActionPriority, ActionType, RecoveryAction
...
class RecoveryRecommendationEngine:
    ...
    async def recommend_recovery_actions(
        self, customer_id: str, db: AsyncSession, trigger_context: Optional[Dict] = None
    ) -> List[Dict]:
        """
        ...
        prioritized_actions = self._prioritize_actions(
            recommendations, customer, risk_data
        )
    ...
    async def _generate_action_recommendations(
        ...
        if churn_risk >= 0.8:
            recommendations.extend(
                await self._get_critical_churn_actions(customer, trigger_context, db)
            )

        # Rule 2: High churn risk (0.6-0.8)
        elif churn_risk >= 0.6:
            recommendations.extend(
                await self._get_high_churn_actions(customer, trigger_context, db)
            )

        # Rule 3: High bad review risk (>0.7)
        if review_risk >= 0.7:
            recommendations.extend(
                await self._get_review_prevention_actions(customer, trigger_context, db)
            )

        # Rule 4: Recent negative review trigger
        if trigger_context and trigger_context.get("type") == "review":
            recommendations.extend(
                await self._get_review_response_actions(customer, trigger_context, db)
            )

        # Rule 5: Support ticket trigger
        if trigger_context and trigger_context.get("type") == "support_ticket":
            recommendations.extend(
                await self._get_support_recovery_actions(customer, trigger_context, db)
            )

        # Rule 6: High-value customer special handling
        if customer.is_high_value:
            recommendations.extend(
                await self._get_high_value_actions(customer, trigger_context, db)
            )

        # Rule 7: Long-term inactive customer
        if (
            customer.days_since_last_interaction
            and customer.days_since_last_interaction > 90
        ):
            recommendations.extend(
                await self._get_reactivation_actions(customer, trigger_context, db)
            )

        return recommendations

    async def _get_critical_churn_actions(
        self, customer: Customer, trigger_context: Optional[Dict], db: AsyncSession
    ) -> List[Dict]:
        ...
        actions.append(
            {
                ...
                "metadata": {"callback_type": "retention", "urgency": "high"},
                "confidence": 0.7,
            }
        )
        ...
        actions.append(
            {
                ...
                "metadata": {"email_type": "proactive_support", "tone": "caring"},
                "confidence": 0.7,
            }
        )
    ...
    async def _get_review_response_actions(
        self, customer: Customer, trigger_context: Dict, db: AsyncSession
    ) -> List[Dict]:
        ...
        actions.append(
            {
                ...
                "metadata": {"campaign_type": "win_back", "special_offer": True},
                "confidence": 0.6,
            }
        )
    ...
    def _prioritize_actions(
        self, recommendations: List[Dict], customer: Customer, risk_data: Dict
    ) -> List[Dict]:
        ...
        for action in recommendations:
            ...
            risk_multiplier = 1.0 + max(
                risk_data["churn_risk"], risk_data["bad_review_likelihood"]
            )

            # Calculate final priority score (higher is better)
            priority_score = (
                effectiveness * confidence * value_multiplier * risk_multiplier
            ) / (cost / 10)
    ...
    def _schedule_actions(
        self, prioritized_actions: List[Dict], customer: Customer
    ) -> List[Dict]:
    ...
    async def get_pending_actions(
        self, organization_id: str, db: AsyncSession, limit: int = 100
    ) -> List[RecoveryAction]:

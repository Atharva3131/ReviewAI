...
from app.schemas.review import ReviewIngest, ReviewFilter, ReviewStats
...
class ReviewService:
    ...
    async def ingest_review(
        ...
        if review_data.customer_email:
            customer = await ReviewService._find_or_create_customer(
                db, organization_id, review_data.customer_email, review_data.customer_name
            )
    ...
    async def get_reviews(
        ...
        if filters:
            ...
            if filters.requires_private_recovery is not None:
                query = query.where(Review.requires_private_recovery == filters.requires_private_recovery)
    ...
    async def update_review(
        ...
        allowed_fields = {
            "sentiment_score", "urgency_level", "issue_categories",
            "status", "requires_private_recovery", "public_response",
            "internal_notes", "processed_at"
        }
    ...
    async def get_review_stats(
        ...
        avg_rating_result = await db.execute(
            select(func.avg(Review.rating)).where(Review.organization_id == organization_id)
        )
        ...
        rating_distribution = {str(rating): count for rating, count in rating_dist_result}
        ...
        status_distribution = {status.value: count for status, count in status_dist_result}
        ...
        response_rate = (responded_count.scalar() / total_reviews) * 100 if total_reviews > 0 else 0.0
        ...
        private_recovery_rate = (recovery_count.scalar() / total_reviews) * 100 if total_reviews > 0 else 0.0
    ...
    async def mark_review_processed(
        ...
        review.requires_private_recovery = analysis_results.get("requires_private_recovery", False)

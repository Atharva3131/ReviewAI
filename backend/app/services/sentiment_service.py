...
class SentimentService:
    ...
    SERVICE_PATTERNS = {
        ...
        r"will (?:not )?(?:return|come back)": lambda m: (
            -0.3 if "not" in m.group() else 0.2
        ),
    }
    ...
    async def analyze_sentiment(content: str, rating: int = None) -> Dict[str, Any]:
        ...
        while i < len(words):
            ...
            if word_score is not None:
                ...
                word_score = SentimentService._apply_intensity(
                    word_score, intensity_modifier
                )
        ...
        dominant_emotion = SentimentService._determine_emotion(
            final_sentiment, word_scores
        )
    ...
    def _check_negation_context(words: List[str], index: int) -> bool:
        ...
        return any(
            word.lower() in SentimentService.NEGATION_WORDS for word in context_words
        )
    ...
    def _calculate_confidence(
        scored_words: int, total_words: int, rating: int, sentiment: float
    ) -> float:
    ...
    @staticmethod
    def _create_result(
        sentiment: float, confidence: float, emotion: str, word_scores: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    async def check_rating_sentiment_correlation(
        rating: int, sentiment_score: float, tolerance: float = 0.3
    ) -> Dict[str, Any]:

...
class SupportTicketService:
    ...
    async def create_ticket(
        self, organization_id: UUID, ticket_data: SupportTicketCreate
    ) -> SupportTicket:
    ...
    async def get_ticket(self, ticket_id: UUID, organization_id: UUID) -> SupportTicket:
    ...
    async def update_ticket(
        self, ticket_id: UUID, organization_id: UUID, update_data: SupportTicketUpdate
    ) -> SupportTicket:
    ...
    async def assign_ticket(
        self, ticket_id: UUID, organization_id: UUID, assigned_to: str
    ) -> SupportTicket:
    ...
    async def resolve_ticket(
        self, ticket_id: UUID, organization_id: UUID, resolution: str, resolved_by: str
    ) -> SupportTicket:
    ...
    async def close_ticket(
        self, ticket_id: UUID, organization_id: UUID
    ) -> SupportTicket:
    ...
    async def reopen_ticket(
        self, ticket_id: UUID, organization_id: UUID, reason: Optional[str] = None
    ) -> SupportTicket:
    ...
    async def get_ticket_stats(self, organization_id: UUID) -> TicketStatsResponse:
        ...
        # Average times
        response_times = [
            t.time_to_first_response for t in tickets if t.time_to_first_response
        ]
        resolution_times = [
            t.time_to_resolution for t in tickets if t.time_to_resolution
        ]
        satisfaction_ratings = [
            t.satisfaction_rating for t in tickets if t.satisfaction_rating
        ]
        ...
        avg_satisfaction_rating = (
            sum(satisfaction_ratings) / len(satisfaction_ratings)
        ...
        for priority in TicketPriority:
            tickets_by_priority[priority.value] = sum(
        ...
        for category in TicketCategory:
            tickets_by_category[category.value] = sum(
                1 for t in tickets if t.category == category
            )
        ...
        for ticket in tickets:
            if ticket.source:
                tickets_by_source[ticket.source] = (
                    tickets_by_source.get(ticket.source, 0) + 1
                )
        ...
        return TicketStatsResponse(
            ...
            avg_time_to_first_response=(
                round(avg_time_to_first_response, 2)
                if avg_time_to_first_response
                else None
            ),
            avg_time_to_resolution=(
                round(avg_time_to_resolution, 2) if avg_time_to_resolution else None
            ),
            avg_satisfaction_rating=(
                round(avg_satisfaction_rating, 2) if avg_satisfaction_rating else None
            ),
    ...
    async def delete_ticket(self, ticket_id: UUID, organization_id: UUID) -> None:

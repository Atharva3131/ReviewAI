...
class UrgencyService:
    ...
    HIGH_URGENCY_KEYWORDS = {
        ...
        "scam": 0.8,
        "stolen": 0.9,
        "unauthorized charge": 0.8,
        "billing error": 0.6,
        # Time-sensitive issues
        "deadline": 0.7,
        "time sensitive": 0.8,
        "expires": 0.6,
        "running out of time": 0.7,
        "last minute": 0.6,
    }
    ...
    MEDIUM_URGENCY_KEYWORDS = {
        ...
        "ignored": 0.6,
        "unresponsive": 0.6,
        "poor communication": 0.5,
        "lack of information": 0.5,
    }
    ...
    async def classify_urgency(
        ...
        if not content or not content.strip():
            return UrgencyService._create_result(UrgencyLevel.LOW, 0.1, 0.0, [], [])
        ...
        pattern_score, contributing_factors = UrgencyService._analyze_patterns(
            full_text
        )
    ...
    def _analyze_keywords(content: str) -> Tuple[float, List[str]]:
        ...
        low_urgency_count = sum(
            1
            for indicator in UrgencyService.LOW_URGENCY_INDICATORS
        ...
        if word_count > 0:
            normalized_score = min(
                total_score / (word_count / 50), 1.0
            )  # Normalize per 50 words
        else:
    ...
    def _calculate_confidence(
        ...
        high_urgency_matches = len(
            [m for m in keyword_matches if m.startswith("HIGH:")]
        )
        medium_urgency_matches = len(
            [m for m in keyword_matches if m.startswith("MEDIUM:")]
        )
        ...
        if sentiment_score is not None:
            expected_urgency = UrgencyService._calculate_sentiment_urgency(
                sentiment_score
            )
            consistency = 1.0 - abs(urgency_score - expected_urgency)
    ...
    async def validate_urgency_classification(
        content: str, predicted_level: UrgencyLevel, rating: int = None
    ) -> Dict[str, Any]:
        ...
        if rating and rating >= 4 and predicted_level == UrgencyLevel.HIGH:
            validation_issues.append(
                "High urgency with positive rating may be inconsistent"
            )

        # Very short content with high urgency
        if len(content.split()) < 5 and predicted_level == UrgencyLevel.HIGH:
            validation_issues.append(
                "High urgency classification with very short content"
            )

        # No urgency keywords but high classification
        if predicted_level == UrgencyLevel.HIGH and not any(
            "HIGH:" in match for match in analysis["keyword_matches"]
        ):

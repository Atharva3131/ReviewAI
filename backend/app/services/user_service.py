...
class UserService:
    ...
    async def get_user_by_id(
        ...
        try:
            ...
            result = await db.execute(
                select(User).where(
                    and_(User.id == user_uuid, User.organization_id == organization_id)
                )
            )
    ...
    async def deactivate_user(
        ...
        if not PermissionManager.has_permission(
            deactivated_by_role, Permission.USER_DELETE
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def reactivate_user(
        ...
        # Validate permissions
        if not PermissionManager.has_permission(
            reactivated_by_role, Permission.USER_UPDATE
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    def _can_change_role(
        changer_role: UserRole, current_role: UserRole, new_role: UserRole
    ) -> bool:
    ...
    async def invite_user(
        ...
        # Validate permissions
        if not PermissionManager.has_permission(
            invited_by_role, Permission.USER_CREATE
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,

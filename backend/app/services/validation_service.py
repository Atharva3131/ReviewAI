...
class ValidationService:
    ...
    SPAM_KEYWORDS = {
        ...
        'buy now', 'limited time', 'act now', 'guaranteed', 'risk free',
        'no obligation', 'call now', 'order now', 'visit our website'
    }
    ...
    def validate_review_content(
        ...
        if not content or not content.strip():
            if ValidationService.PLATFORM_RULES[platform].get('require_rating') and rating:
        ...
        if len(content) < min_length:
            validation_result['errors'].append(f'Review content too short (minimum {min_length} characters)')
            validation_result['is_valid'] = False
        
        if len(content) > max_length:
            validation_result['errors'].append(f'Review content too long (maximum {max_length} characters)')
            validation_result['is_valid'] = False
        
        # Sanitize content
        sanitized = ValidationService._sanitize_content(content, platform_rules.get('allow_html', False))
        ...
        if spam_score > 0.7:
            ...
            validation_result['warnings'].append('Content may contain promotional material')
        ...
        if ValidationService._contains_excessive_profanity(content):
            validation_result['warnings'].append('Review contains inappropriate language')
    ...
    def validate_customer_info(
        ...
        if customer_name:
            ...
            if not re.match(r'^[a-zA-Z\s\-\.\']+$', sanitized_name):
                validation_result['warnings'].append('Customer name contains unusual characters')
    ...
    def _calculate_spam_score(content: str) -> float:
        ...
        spam_keyword_count = sum(1 for keyword in ValidationService.SPAM_KEYWORDS 
    ...
    def validate_external_id(external_id: str, platform: ReviewPlatform) -> bool:
        ...
        if platform == ReviewPlatform.GOOGLE:
            ...
            return len(external_id) > 10 and external_id.replace('-', '').replace('_', '').isalnum()
        ...
        return len(external_id) <= 255 and external_id.replace('-', '').replace('_', '').isalnum()

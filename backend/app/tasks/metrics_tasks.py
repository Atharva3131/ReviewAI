...
async def _aggregate_metrics_async(organization_id: Optional[str]) -> Dict[str, Any]:
    ...
    async with get_async_db_context() as db:
        ...
        # Base query filter
        org_filter = Customer.organization_id == organization_id if organization_id else True
        
        # 1. Review Metrics
        review_metrics = await _calculate_review_metrics(db, org_filter, thirty_days_ago)
        
        # 2. Customer Metrics
        customer_metrics = await _calculate_customer_metrics(db, org_filter)
        
        # 3. Recovery Action Metrics
        recovery_metrics = await _calculate_recovery_metrics(db, org_filter, thirty_days_ago)
        
        # 4. Support Ticket Metrics
        support_metrics = await _calculate_support_metrics(db, org_filter, thirty_days_ago)
        
        # 5. Performance Metrics
        performance_metrics = await _calculate_performance_metrics(db, org_filter, thirty_days_ago)
...
async def _calculate_review_metrics(db: AsyncSession, org_filter, start_date: datetime) -> Dict[str, Any]:
    ...
    rating_distribution = {str(rating): count for rating, count in rating_dist_result.fetchall()}
    
    # Sentiment metrics
    sentiment_result = await db.execute(
        select(
            func.avg(Review.sentiment_score).label("avg_sentiment"),
            func.count(Review.id).filter(Review.sentiment_score < 0.4).label("negative_count"),
            func.count(Review.id).filter(Review.sentiment_score >= 0.6).label("positive_count")
        )
        .join(Customer)
        .where(and_(org_filter, Review.created_at >= start_date, Review.sentiment_score.isnot(None)))
    )
    ...
    response_result = await db.execute(
        select(
            func.count(Review.id).filter(Review.status == "responded").label("responded_count"),
            func.count(Review.id).filter(Review.ai_response.isnot(None)).label("ai_generated_count")
        )
        .join(Customer)
        .where(and_(org_filter, Review.created_at >= start_date))
    )
    ...
    return {
        ...
        "response_rate": round((response_data.responded_count or 0) / max(total_reviews, 1), 3)
    }
...
async def _calculate_customer_metrics(db: AsyncSession, org_filter) -> Dict[str, Any]:
    ...
    high_value_result = await db.execute(
        select(func.count(Customer.id))
        .where(and_(org_filter, Customer.lifetime_value >= 1000))
    )
    high_value_customers = high_value_result.scalar() or 0
    
    # Customer satisfaction
    satisfaction_result = await db.execute(
        select(func.avg(Customer.avg_rating_given))
        .where(and_(org_filter, Customer.avg_rating_given.isnot(None)))
    )
    ...
    new_customers_result = await db.execute(
        select(func.count(Customer.id))
        .where(and_(org_filter, Customer.created_at >= thirty_days_ago))
    )
    ...
    return {
        ...
        "at_risk_percentage": round((at_risk_customers / max(total_customers, 1)) * 100, 1)
    }


async def _calculate_recovery_metrics(db: AsyncSession, org_filter, start_date: datetime) -> Dict[str, Any]:
    ...
    success_result = await db.execute(
        select(
            func.count(RecoveryAction.id).filter(RecoveryAction.success == True).label("successful"),
            func.count(RecoveryAction.id).filter(RecoveryAction.customer_responded == True).label("responded"),
    ...
    action_type_distribution = {action_type.value: count for action_type, count in action_type_result.fetchall()}
    ...
    return {
        ...
        "success_rate": round((success_data.successful or 0) / max(total_actions, 1), 3),
        "response_rate": round((success_data.responded or 0) / max(total_actions, 1), 3),
        "average_effectiveness": round(float(success_data.avg_effectiveness or 0), 3),
        "action_type_distribution": action_type_distribution
    }


async def _calculate_support_metrics(db: AsyncSession, org_filter, start_date: datetime) -> Dict[str, Any]:
    ...
    priority_distribution = {priority.value: count for priority, count in priority_result.fetchall()}
    
    return {
        "total_tickets": total_tickets,
        "status_distribution": status_distribution,
        "priority_distribution": priority_distribution,
        "resolution_rate": round(status_distribution.get("resolved", 0) / max(total_tickets, 1), 3)
    }


async def _calculate_performance_metrics(db: AsyncSession, org_filter, start_date: datetime) -> Dict[str, Any]:
    ...
    recovery_time_result = await db.execute(
        select(func.avg(
            func.extract('epoch', RecoveryAction.executed_at - RecoveryAction.created_at)
        ))
        .join(Customer)
        .where(and_(
            org_filter,
...
def batch_update_risk_scores(self, organization_id: str, limit: int = 50) -> Dict[str, Any]:
    """
...
async def _batch_update_risk_scores_async(organization_id: str, limit: int) -> Dict[str, Any]:
...
async def _calculate_kpis_async(organization_id: str) -> Dict[str, Any]:
    ...
    async with get_async_db_context() as db:
        ...
        recovery_success_rate = (recovery_data.responded / max(recovery_data.total, 1)) if recovery_data.total else 0
...
@celery_app.task(bind=True)
def generate_analytics_report(self, organization_id: str, report_type: str = "monthly") -> Dict[str, Any]:
    """
    ...
    """
    try:
        return asyncio.run(_generate_analytics_report_async(organization_id, report_type))
...
async def _generate_analytics_report_async(organization_id: str, report_type: str) -> Dict[str, Any]:

...
def log_failed_task(
    self, task_id: str, error: str, traceback_str: str
) -> Dict[str, Any]:
    """
...
def retry_failed_task(
    self, original_task_name: str, task_args: List, task_kwargs: Dict
) -> Dict[str, Any]:
    """
...
def monitor_queue_health(self) -> Dict[str, Any]:
    ...
    """
    try:
        from app.core.celery import get_active_tasks, get_queue_length
        ...
        for queue_name, threshold in queue_thresholds.items():
            try:
                ...
                if not is_healthy:
                    ...
                    logger.warning(
                        f"Queue {queue_name} is unhealthy: {queue_length} tasks (threshold: {threshold})"
                    )

            except Exception as e:
                queue_health[queue_name] = {"error": str(e), "healthy": False}
                overall_healthy = False
        ...
        try:
            ...
            total_active = sum(
                len(tasks) for tasks in active_tasks.get("active", {}).values()
            )
        except Exception as e:
    ...
    try:
        ...
        for task_type, metrics in performance_data["performance_metrics"].items():
            if metrics["success_rate"] < 0.95:
                issues.append(
                    f"{task_type} has low success rate: {metrics['success_rate']}"
                )
            if metrics["avg_execution_time"] > 120:
                issues.append(
                    f"{task_type} has high execution time: {metrics['avg_execution_time']}s"
                )
...
def generate_monitoring_report(self, report_type: str = "daily") -> Dict[str, Any]:
    ...
    try:
        ...
        report_data = {
            ...
            "report_period": {"start": start_time.isoformat(), "end": now.isoformat()},
        ...
        current_task.update_state(
            state="FAILURE", meta={"error": str(exc), "report_type": report_type}
        )
        raise exc

...
from app.models.recovery_action import ActionStatus, RecoveryAction
    ...
    try:
        ...
        celery_app.send_task(
            ...
            args=[
                self.request.id,
                str(exc),
                f"Recovery action {action_id} execution failed",
            ],
            queue="dead_letter",
        )
    ...
    async with get_async_db_context() as db:
        ...
        if decision["action"] in ["recover_private", "escalate"]:
            ...
            recommendations = await recommendation_engine.recommend_recovery_actions(
                str(review.customer_id), db, trigger_context
            )

            # Create recovery actions
            actions = await recommendation_engine.create_recovery_actions(
                str(review.customer_id), recommendations, db, trigger_context
            )
            ...
            for action in actions:
                if action.priority.value in ["urgent", "high"]:
                    ...
                    delay = (
                        3600 if action.priority.value == "medium" else 7200
                    )  # 1-2 hours
                    celery_app.send_task(
                        "app.tasks.recovery_tasks.execute_recovery_action",
                        args=[str(action.id)],
...
def check_action_effectiveness(self, action_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        current_task.update_state(
            state="FAILURE", meta={"error": str(exc), "action_id": action_id}
        )
        raise exc
    ...
    async with get_async_db_context() as db:
        ...
        if new_reviews:
            ...
            if avg_rating > 3:
                ...
                response_indicators.append(
                    f"Positive review (avg rating: {avg_rating:.1f})"
                )
...
def batch_execute_actions(self, action_ids: List[str]) -> Dict[str, Any]:
    ...
    """
    results = {"total": len(action_ids), "scheduled": 0, "failed": 0, "results": []}
    ...
    for action_id in action_ids:
        try:
            ...
            results["results"].append(
                {"action_id": action_id, "task_id": task.id, "status": "scheduled"}
            )
            results["scheduled"] += 1

        except Exception as e:
            results["results"].append(
                {"action_id": action_id, "status": "failed", "error": str(e)}
            )
            results["failed"] += 1
...
def generate_recovery_report(
    ...
    try:
        ...
        current_task.update_state(
            state="FAILURE",
            meta={"error": str(exc), "organization_id": organization_id, "days": days},
        )
        raise exc


async def _generate_recovery_report_async(
    organization_id: str, days: int
) -> Dict[str, Any]:
    ...
    async with get_async_db_context() as db:
        ...
        success_rate = (
            (stats.successful_actions / total_actions) if total_actions > 0 else 0
        )
        response_rate = (
            (stats.customer_responses / total_actions) if total_actions > 0 else 0
        )

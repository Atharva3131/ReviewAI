...
async def _get_or_create_customer(db: AsyncSession, review_data: Dict[str, Any]) -> Customer:
...
async def _analyze_review(review: Review, db: AsyncSession) -> Dict[str, Any]:
    ...
    confidence_score = (
        ...
        (sum(cat.get("confidence", 0.8) for cat in categories) / max(len(categories), 1)) * 0.3
    )
...
def generate_review_response(self, review_id: str, auto_publish: bool = False) -> Dict[str, Any]:
    """
...
async def _generate_review_response_async(review_id: str, auto_publish: bool) -> Dict[str, Any]:
    ...
    async with get_async_db_context() as db:
        ...
        response = await llm_service.generate_review_response(
            ...
            customer_name=review.customer.display_name if review.customer else "Valued Customer",

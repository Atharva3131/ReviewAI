...
from app.models.recovery_action import ActionType, RecoveryAction
...
class MockServices:
    ...
    def create_mock_llm_service():
        ...
        mock_service.generate_apology_message.return_value = (
            "We apologize for the service issues you encountered."
        )
        mock_service.generate_discount_offer.return_value = (
            "As an apology, we'd like to offer you a 15% discount on your next order."
        )
        return mock_service
...
class DatabaseTestHelper:
    ...
    async def create_test_user(
        session: AsyncSession, organization: Organization, **kwargs
    ) -> User:
    ...
    @staticmethod
    async def create_test_support_ticket(
        session: AsyncSession, organization: Organization, customer: Customer, **kwargs
    ) -> SupportTicket:
    ...
    @staticmethod
    async def create_test_recovery_action(
        session: AsyncSession, organization: Organization, customer: Customer, **kwargs
    ) -> RecoveryAction:
    ...
    @staticmethod
    async def create_test_agent_decision(
        session: AsyncSession, organization: Organization, input_id: uuid.UUID, **kwargs
    ) -> AgentDecision:
...
class TestScenarios:
    ...
    async def create_high_risk_customer_scenario(session: AsyncSession) -> Dict:
        ...
        customer = await DatabaseTestHelper.create_test_customer(
            session, organization, churn_risk_score=0.95, bad_review_likelihood=0.88
        )
        ...
        for action_type in [ActionType.EMAIL, ActionType.CALL, ActionType.DISCOUNT]:
            action = await DatabaseTestHelper.create_test_recovery_action(
                session, organization, customer, action_type=action_type
            )
...
class PerformanceTestHelper:
    ...
    def assert_performance_threshold(
        execution_time: float, threshold: float, operation: str
    ):

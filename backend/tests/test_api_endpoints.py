...
class TestReviewsAPI:
    ...
    def test_ingest_review_success(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.ingest_review', return_value=mock_review), \
            ...
            response = client.post("/api/v1/reviews/ingest", json={
                "platform": "google",
    ...
    def test_analyze_review_success(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_review_by_id', return_value=mock_review), \
             patch('app.services.sentiment_service.SentimentService.analyze_sentiment', return_value=sentiment_result), \
             patch('app.services.urgency_service.UrgencyService.classify_urgency', return_value=urgency_result), \
             patch('app.services.categorization_service.CategorizationService.categorize_issues', return_value=categorization_result), \
             patch('app.services.review_service.ReviewService.mark_review_processed'):
            
            response = client.post("/api/v1/reviews/analyze", json={
    ...
    def test_analyze_review_not_found(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_review_by_id', return_value=None):
            response = client.post("/api/v1/reviews/analyze", json={
    ...
    def test_generate_review_response_success(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_review_by_id', return_value=mock_review):
            response = client.post("/api/v1/reviews/respond", json={
                "review_id": "review-123",
    ...
    def test_get_reviews_success(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_reviews', return_value=mock_reviews), \
             patch('app.schemas.review.ReviewResponse.from_orm', side_effect=[
    ...
    def test_get_reviews_with_filters(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_reviews', return_value=[]), \
             patch('app.schemas.review.ReviewResponse.from_orm', return_value=[]):
            
            response = client.get("/api/v1/reviews/?platform=google&rating_min=4&rating_max=5")
    ...
    def test_get_review_stats_success(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_review_stats', return_value=mock_stats):
...
class TestDashboardAPI:
    ...
    def test_get_dashboard_metrics_success(self, client, mock_dependencies):
        ...
        with patch('app.core.dependencies.require_organization_access', return_value="org-123"), \
             patch('app.services.dashboard_service.DashboardMetricsService') as mock_service_class:
    ...
    def test_get_dashboard_metrics_with_time_range(self, client, mock_dependencies):
        ...
        with patch('app.core.dependencies.require_organization_access', return_value="org-123"), \
            ...
            for time_range in ["7d", "30d", "90d", "1y"]:
                response = client.get(f"/api/v1/dashboard/metrics?time_range={time_range}")
                assert response.status_code == status.HTTP_200_OK
...
class TestAuthAPI:
    ...
    def test_login_success(self, client):
        ...
        with patch('app.services.auth_service.AuthService.authenticate_user', return_value=mock_user), \
             patch('app.services.auth_service.AuthService.create_access_token', return_value=mock_token):
    ...
    def test_register_email_exists(self, client):
        ...
        with patch('app.services.auth_service.AuthService.register_user', side_effect=ValueError("Email already exists")):
            response = client.post("/api/v1/auth/register", json={
...
class TestCustomersAPI:
    ...
    def test_get_customers_success(self, client, mock_dependencies):
        ...
        with patch('app.services.customer_service.CustomerService.get_customers', return_value=mock_customers), \
             patch('app.schemas.customer.CustomerResponse.from_orm', side_effect=[
    ...
    def test_get_customer_by_id_success(self, client, mock_dependencies):
        ...
        with patch('app.services.customer_service.CustomerService.get_customer_by_id', return_value=mock_customer), \
             patch('app.schemas.customer.CustomerResponse.from_orm', return_value={
                 "id": "customer-123",
                 "name": "John Doe",
    ...
    def test_customer_recovery_success(self, client, mock_dependencies):
        ...
        with patch('app.services.customer_recovery_service.CustomerRecoveryService.create_recovery_plan', return_value=mock_recovery_plan):
            response = client.post("/api/v1/customers/customer-123/recover", json={
...
class TestAgentsAPI:
    ...
    def test_agent_decide_action_success(self, client, mock_dependencies):
        ...
        with patch('app.services.agent_engine.AgentEngine.decide_action', return_value=mock_decision):
            response = client.post("/api/v1/agents/decide-action", json={
                "input_type": "review",
                "input_id": "review-123",
                "context": {
                    "sentiment_score": 0.2,
...
class TestErrorHandling:
    ...
    def test_unauthorized_access(self, client):
        ...
        with patch('app.core.dependencies.get_current_user', side_effect=HTTPException(status_code=401, detail="Not authenticated")):
            response = client.get("/api/v1/reviews/")
            assert response.status_code == status.HTTP_401_UNAUTHORIZED
    
    def test_forbidden_access(self, client, mock_dependencies):
        """Test forbidden access to organization resources"""
        with patch('app.core.dependencies.get_access_control_context', side_effect=HTTPException(status_code=403, detail="Forbidden")):
            response = client.get("/api/v1/reviews/")
            assert response.status_code == status.HTTP_403_FORBIDDEN
    
    def test_internal_server_error(self, client, mock_dependencies):
        """Test internal server error handling"""
        with patch('app.services.review_service.ReviewService.get_reviews', side_effect=Exception("Database error")):
            response = client.get("/api/v1/reviews/")
...
class TestPagination:
    ...
    def test_pagination_default(self, client, mock_dependencies):
        ...
        with patch('app.services.review_service.ReviewService.get_reviews', return_value=[]) as mock_service:
            response = client.get("/api/v1/reviews/")

...
from app.schemas.auth import TokenResponse, UserLogin, UserRegistration
...
class TestAuthService:
    ...
    async def test_register_user_success(self):
        ...
        with (
            patch("app.models.organization.Organization", return_value=mock_new_org),
            patch("app.models.user.User", return_value=mock_new_user),
        ):

            user, organization = await AuthService.register_user(
    ...
    async def test_authenticate_user_success(self):
        ...
        user, organization = await AuthService.authenticate_user(
    ...
    async def test_create_tokens_success(self):
        ...
        with (
            patch(
                "app.core.security.SecurityService.create_access_token",
                return_value="access_token",
            ),
            patch(
                "app.core.security.SecurityService.create_refresh_token",
                return_value="refresh_token",
            ),
            patch("app.core.redis.redis_client.set", new_callable=AsyncMock),
            patch(
                "app.schemas.auth.UserResponse.from_orm",
                return_value={"id": str(self.mock_user.id)},
            ),
            patch(
                "app.schemas.auth.OrganizationResponse.from_orm",
                return_value={"id": str(self.mock_org.id)},
            ),
            ...
            token_response = await AuthService.create_tokens(
                self.mock_user, self.mock_org
            )
    ...
    async def test_refresh_access_token_success(self):
        ...
        with (
            patch(
                "app.core.security.SecurityService.verify_token",
                return_value=mock_payload,
            ),
            patch("app.core.redis.redis_client.get", return_value=refresh_token),
            patch.object(
                AuthService, "create_tokens", return_value=MagicMock()
    ...
    async def test_logout_user_failure(self):
        ...
        with patch(
            "app.core.redis.redis_client.delete", side_effect=Exception("Redis error")
        ):
            result = await AuthService.logout_user(user_id)
            assert result == False
    ...
    async def test_reset_password_success(self):
        ...
        with (
            patch(
                "app.core.security.SecurityService.generate_password_reset_token",
    ...
    async def test_confirm_password_reset_success(self):
        ...
        with (
            patch("app.core.redis.redis_client.get", return_value=user_id),
...
class TestAuthenticationIntegration:
    ...
    async def test_full_authentication_flow(self):
        ...
        # Test registration
        with (
            patch("app.models.organization.Organization", return_value=mock_org),
    ...
    async def test_token_refresh_flow(self):
        ...
        with (
            ...
            patch("app.core.redis.redis_client.set"),
            patch(
                "app.schemas.auth.UserResponse.from_orm", return_value={"id": user_id}
            ),
            patch(
                "app.schemas.auth.OrganizationResponse.from_orm",
                return_value={"id": mock_org.id},
            ),
            patch("app.core.config.settings") as mock_settings,
        ):
            ...
            with (
                ...
                patch("app.core.redis.redis_client.get", return_value=refresh_token),
                patch.object(
                    AuthService, "create_tokens", return_value=token_response
                ) as mock_create_tokens,
            ):
...
def create_test_login_data():
    ...
    return UserLogin(email="test@example.com", password="SecurePassword123!")

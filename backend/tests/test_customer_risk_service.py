...
class TestCustomerRiskAssessmentService:
    ...
    async def test_assess_customer_risk_high_risk(self):
        ...
        self.mock_db.execute.return_value.scalar.return_value = (
            0  # No recent interactions
        )
        ...
        with patch.object(
            self.service, "_get_customer_with_data", return_value=customer
        ):
            result = await self.service.assess_customer_risk(
                "customer-123", self.mock_db
            )

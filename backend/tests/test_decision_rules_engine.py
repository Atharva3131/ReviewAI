...
from app.services.decision_rules_engine import DecisionRule, DecisionRulesEngine
...
class TestDecisionRulesEngine:
    ...
    def test_requires_approval(self):
        ...
        assert (
            self.engine._requires_approval(DecisionType.RECOVER_PRIVATE, 0.9) == False
        )
    ...
    def test_add_custom_rule(self):
        ...
        custom_condition = lambda ctx: ctx["rating"] == 3 and "test" in ctx.get(
            "content", ""
        )
    ...
    async def test_comprehensive_rule_coverage(self):
        ...
        for scenario in test_scenarios:
            result = await self.engine.decide_review_action(
                **scenario, context={"content": "Test scenario"}
            )

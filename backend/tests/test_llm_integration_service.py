...
class TestContentSanitizer:
    ...
    def test_sanitize_content_with_sensitive_data(self):
        ...
        content = (
            "My credit card number is 1234-5678-9012-3456 and password is secret123"
        )
...
class TestLLMResponseGenerationService:
    ...
    async def test_generate_review_response_success(self):
        ...
        with (
            ...
            result = await self.service.generate_review_response(
                rating=5, review_content="Great service!", customer_name="John Doe"
            )
    ...
    async def test_generate_response_with_caching(self):
        ...
        with (
            ...
            result1 = await self.service.generate_review_response(
                rating=5, review_content="Great service!", use_cache=True
            )
    ...
    async def test_generate_response_llm_error(self):
        ...
        error_provider.generate_response.side_effect = LLMError(
            "API Error", LLMProvider.MOCK
        )
    ...
    def test_clear_cache(self):
        ...
        request = GenerationRequest(
            template_type=TemplateType.REVIEW_RESPONSE, variables={"rating": 5}
        )
    ...
    def test_select_template(self):
        ...
        with patch.object(
            self.service.template_manager,
            "get_templates_by_type",
            return_value=[inactive_template],
        ):
            with pytest.raises(ValueError, match="No active templates found"):
    ...
    def test_get_content_type(self):
        ...
        assert (
            self.service._get_content_type(TemplateType.REVIEW_RESPONSE)
            == "review_response"
        )
        ...
        assert (
            self.service._get_content_type(TemplateType.GENERIC_RESPONSE) == "general"
        )
    ...
    async def test_generate_response_with_custom_provider(self):
        ...
        with (
            ...
            patch(
                "app.services.llm.response_generation_service.get_llm_provider",
                return_value=custom_provider,
            ),
        ):

            result = await self.service.generate_review_response(
                rating=5, review_content="Great service!", provider=LLMProvider.OPENAI
            )
    ...
    async def test_generate_response_with_metadata(self):
        ...
        with (
            ...
            result = await self.service.generate_review_response(
                rating=5, review_content="Great service!", metadata=custom_metadata
            )

...
class TestDatabaseMigrator:
    ...
    def test_migrator_requires_database_url(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(MigrationError, match="DATABASE_URL environment variable is not set"):
                DatabaseMigrator()
    ...
    def test_validate_migrations_syntax_error(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'run_command', return_value=(1, '', 'SyntaxError')):
                with patch('os.path.exists', return_value=True):
    ...
    def test_check_alembic_config_failure(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'run_command', return_value=(1, '', 'Config error')):
                result = migrator.check_alembic_config()
                assert result is False
    ...
    def test_get_current_revision(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'run_command', return_value=(0, '001 (head)\n', '')):
    ...
    def test_generate_sql(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'run_command', return_value=(0, sql_output, '')):
    ...
    def test_verify_migration_failure(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'get_current_revision', return_value='001'):
                with patch.object(migrator, 'get_pending_migrations', return_value=['002']):
                    result = migrator.verify_migration()
                    assert result is False
    ...
    def test_create_migration_report_with_error(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(migrator, 'get_current_revision', return_value='001'):
                report = migrator.create_migration_report(success=False, error="Test error")
...
class TestDatabaseRollback:
    ...
    def test_rollback_requires_database_url(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(RollbackError, match="DATABASE_URL environment variable is not set"):
                DatabaseRollback()
    ...
    def test_get_migration_history(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(rollback, 'run_command', return_value=(0, history_output, '')):
    ...
    def test_generate_rollback_sql(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(rollback, 'run_command', return_value=(0, sql_output, '')):
    ...
    def test_run_rollback_dry_run(self):
        ...
        with patch.dict(os.environ, {'DATABASE_URL': 'postgresql://test:test@localhost/test'}):
            ...
            with patch.object(rollback, 'get_current_revision', return_value='002'):
                with patch.object(rollback, 'validate_target_revision', return_value=True):
                    with patch.object(rollback, 'generate_rollback_sql', return_value='SQL'):
            ...
            with patch.object(rollback, 'get_current_revision', return_value='002'):
                with patch.object(rollback, 'validate_target_revision', return_value=True):
                    with patch.object(rollback, 'confirm_rollback', return_value=True):
                        with patch.object(rollback, 'run_command', return_value=(0, 'Success', '')):
                ...
                with patch.object(migrator, 'check_alembic_config', return_value=True):
                    with patch.object(migrator, 'get_pending_migrations', return_value=['002']):
                        with patch.object(migrator, 'run_migration', return_value=True):
                            with patch.object(migrator, 'verify_migration', return_value=True):
            ...
            with patch.object(rollback, 'get_current_revision', return_value='002'):
                with patch.object(rollback, 'validate_target_revision', return_value=True):
                    with patch.object(rollback, 'run_rollback', return_value=True):
                        with patch.object(rollback, 'verify_rollback', return_value=True):
                            # Simulate full workflow

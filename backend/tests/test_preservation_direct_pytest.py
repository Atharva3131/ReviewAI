...
def test_pytest_uses_pyproject_toml_configuration():
    ...
    **Validates: Requirement 3.1, 3.2**
    ...
    assert (
        "slow:" in result.stdout
        or "integration:" in result.stdout
        or "unit:" in result.stdout
...
def test_pytest_marker_filtering_works():
    ...
    **Validates: Requirement 3.2**
    ...
    assert result.returncode in [
        0,
        5,
    ], f"pytest with marker filter failed unexpectedly. stderr: {result.stderr}"
...
def test_pytest_testpaths_configuration():
    ...
    **Validates: Requirement 3.1**
    ...
    assert result.returncode in [
        0,
        5,
    ], f"pytest --collect-only failed. stderr: {result.stderr}"
    # Should show that it's using pyproject.toml configuration
    assert (
        "pyproject.toml" in result.stdout or "test session starts" in result.stdout
...
def test_property_pytest_accepts_all_configured_markers(marker):
    ...
    **Validates: Requirement 3.2**
    ...
    assert (
        "unknown marker" not in result.stderr.lower()
...
def test_pytest_coverage_configuration_preserved():
    ...
    **Validates: Requirement 3.3**
    ...
    assert (
        "coverage" in result.stdout.lower() or "coverage" in result.stderr.lower()
...
def test_direct_pytest_help_command():
    ...
    **Validates: Requirement 3.6**
    ...
    assert result.returncode == 0, "pytest --help should work"
    assert (
        "usage:" in result.stdout.lower() or "pytest" in result.stdout.lower()
...
def test_property_pytest_accepts_standard_flags(verbose_flag, quiet_flag):
    ...
    **Validates: Requirement 3.6**
    ...
    result = subprocess.run(
        cmd, cwd=BACKEND_DIR, capture_output=True, text=True, timeout=30
    )

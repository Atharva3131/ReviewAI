...
class TestArtifactActionsPreservation:
    ...
    def extract_artifact_configs(
        ...
        for job_name, job_config in workflow["jobs"].items():
            ...
            for step_idx, step in enumerate(job_config["steps"]):
                ...
                uses = step["uses"]
                if (
                    "actions/upload-artifact@" in uses
                    or "actions/download-artifact@" in uses
                ):
        ...
        for artifact in artifacts:
            if artifact["action_type"] == "upload":
                ...
                if "retention-days" in with_params:
                    ...
                    if (
                        workflow_path in expected_retention
                        and artifact_name in expected_retention[workflow_path]
                    ):
        ...
        **Validates: Requirement 3.4**
        ...
        for artifact in artifacts:
            ...
            if if_condition:
                ...
                is_valid = (
                    if_condition in valid_conditions
                    or "always()" in if_condition
                    or "success()" in if_condition
                    or "failure()" in if_condition
                    or "==" in if_condition
                    or "!=" in if_condition
                )
    ...
    def test_workflow_triggers_preserved(self, workflow_path: str):
        ...
        **Validates: Requirement 3.6**
        ...
        assert triggers, f"Workflow {workflow_path} has empty trigger configuration"
    ...
    def _get_expected_retention_periods(

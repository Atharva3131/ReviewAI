...
class TestSentimentAnalysisConsistency:
    ...
    def test_sentiment_score_bounds_property(self, text: str):
        ...
        **Validates: Requirements 4.2.2**
        ...
        assert (
            0.0 <= result["sentiment_score"] <= 1.0
        ), f"Sentiment score {result['sentiment_score']} out of bounds for text: {text[:50]}..."
        assert (
            0.0 <= result["confidence"] <= 1.0
        ), f"Confidence {result['confidence']} out of bounds for text: {text[:50]}..."
    ...
    def test_sentiment_determinism_property(self, text: str):
        ...
        **Validates: Requirements 4.2.1**
        ...
        ), f"Non-deterministic sentiment for: {text[:50]}..."
        assert (
            result1["confidence"] == result2["confidence"]
        ), f"Non-deterministic confidence for: {text[:50]}..."
        assert (
            result1["dominant_emotion"] == result2["dominant_emotion"]
        ), f"Non-deterministic emotion for: {text[:50]}..."
        assert (
            result1["sentiment_label"] == result2["sentiment_label"]
        ), f"Non-deterministic label for: {text[:50]}..."

    @given(st.text(min_size=1, max_size=200), st.integers(min_value=1, max_value=5))
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_rating_sentiment_correlation_property(self, text: str, rating: int):
        ...
        **Validates: Requirements 4.2.3**
        ...
        result_with_rating = asyncio.run(
            SentimentService.analyze_sentiment(text, rating)
        )
        ...
        if rating == 5:  # 5-star rating
            assert (
                result_with_rating["sentiment_score"] >= 0.3
            ), f"5-star rating should not produce very negative sentiment for: {text[:50]}..."
        elif rating == 1:  # 1-star rating
            assert (
                result_with_rating["sentiment_score"] <= 0.7
            ), f"1-star rating should not produce very positive sentiment for: {text[:50]}..."
    ...
    def test_sentiment_label_consistency_property(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        if score >= 0.8:
            assert (
                label == "Very Positive"
            ), f"Score {score} should be 'Very Positive', got '{label}'"
        elif score >= 0.6:
            assert (
                label == "Positive"
            ), f"Score {score} should be 'Positive', got '{label}'"
        elif score >= 0.4:
            assert (
                label == "Neutral"
            ), f"Score {score} should be 'Neutral', got '{label}'"
        elif score >= 0.2:
            assert (
                label == "Negative"
            ), f"Score {score} should be 'Negative', got '{label}'"
        else:
            assert (
                label == "Very Negative"
            ), f"Score {score} should be 'Very Negative', got '{label}'"
    ...
    def test_case_insensitive_consistency_property(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        assert (
            abs(upper_result["sentiment_score"] - lower_result["sentiment_score"])
            <= tolerance
        ), f"Case sensitivity issue: upper={upper_result['sentiment_score']}, lower={lower_result['sentiment_score']}"

        assert (
            abs(original_result["sentiment_score"] - lower_result["sentiment_score"])
            <= tolerance
    ...
    def test_known_sentiment_words_property(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        if any(word in text.lower() for word in positive_words):
            ...
            ), f"Positive text '{text}' should have positive sentiment, got {result['sentiment_score']}"

        # Property: Strong negative words should produce negative sentiment
        elif any(word in text.lower() for word in negative_words):
            assert (
                result["sentiment_score"] <= 0.4
            ), f"Negative text '{text}' should have negative sentiment, got {result['sentiment_score']}"

        # Property: Neutral words should produce moderate sentiment
        elif any(word in text.lower() for word in neutral_words):
            assert (
                0.3 <= result["sentiment_score"] <= 0.7
            ), f"Neutral text '{text}' should have moderate sentiment, got {result['sentiment_score']}"
    ...
    def test_confidence_calculation_property(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        if word_count >= 20:
            ...
            assert (
                confidence >= 0.1
            ), f"Long text should have some confidence, got {confidence}"
    ...
    **Validates: Requirements 4.2**
    ...
    def cache_consistency(self):
        ...
        for text in self.analyzed_texts:
            assert text in self.results_cache or any(
                text in key for key in self.results_cache.keys()
            )
...
def generate_positive_review_text():
    ...
    return st.text(
        alphabet=st.sampled_from(
            positive_words + [" ", ".", "!", "service", "food", "staff"]
        ),
        min_size=10,
        max_size=100,
    )
...
def generate_negative_review_text():
    ...
    return st.text(
        alphabet=st.sampled_from(
            negative_words + [" ", ".", "!", "service", "food", "staff"]
        ),
        min_size=10,
        max_size=100,
    )
...
def generate_neutral_review_text():
    ...
    return st.text(
        alphabet=st.sampled_from(
            neutral_words + [" ", ".", "service", "food", "staff"]
        ),
        min_size=10,
        max_size=100,
    )
...
class TestSentimentAnalysisRealisticData:
    ...
    def test_positive_review_consistency(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        ), f"Positive text should have positive sentiment: {text[:50]}..."
    ...
    def test_negative_review_consistency(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        assert (
            result["sentiment_score"] <= 0.5
        ), f"Negative text should have negative sentiment: {text[:50]}..."
    ...
    def test_neutral_review_consistency(self, text: str):
        ...
        **Validates: Requirements 4.2**
        ...
        assert (
            0.3 <= result["sentiment_score"] <= 0.7
        ), f"Neutral text should have moderate sentiment: {text[:50]}..."

...
from app.core.validation import InputSanitizer, ValidationError, InputValidator
...
from app.core.sql_protection import SQLSecurityValidator, SQLInjectionError
...
class TestInputValidation:
    ...
    def test_validate_issue_categories(self):
        ...
        for categories in valid_categories:
            ...
            assert all(cat in ["support", "pricing", "delivery", "quality"] for cat in result)
...
class TestSQLProtection:
    ...
    def test_validate_query_string_safe(self):
        ...
        for query in safe_queries:
            ...
            result = SQLSecurityValidator.validate_query_string(
                query, 
                allow_keywords=["SELECT", "FROM", "WHERE", "COUNT"]
            )
    ...
    def test_validate_user_input_contexts(self):
        ...
        result = SQLSecurityValidator.validate_user_input(valid_filter, context="filter")
        assert "active user" in result
        
        # Search context
        valid_search = "customer feedback review"
        result = SQLSecurityValidator.validate_user_input(valid_search, context="search")
...
class TestRateLimiting:
    ...
    async def test_rate_limit_check_within_limits(self):
        ...
        with patch('app.core.rate_limiting.redis_client') as mock_redis:
            ...
            mock_redis.redis.pipeline.return_value.__aenter__.return_value.execute.return_value = [None, 5, None, None]
    ...
    async def test_rate_limit_check_exceeded(self):
        ...
        with patch('app.core.rate_limiting.redis_client') as mock_redis:
            ...
            mock_redis.redis.pipeline.return_value.__aenter__.return_value.execute.return_value = [None, 150, None, None]

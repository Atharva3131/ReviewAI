...
class TestUrgencyService:
    ...
    async def test_classify_urgency_high_legal_threat(self):
        ...
        content = (
            "I'm going to sue you and contact my lawyer about this terrible service!"
        )
        rating = 2
    ...
    async def test_classify_urgency_sentiment_correlation(self):
        ...
        result_negative = await UrgencyService.classify_urgency(
            content, sentiment_score=0.1
        )
        result_positive = await UrgencyService.classify_urgency(
    ...
    async def test_classify_urgency_temporal_indicators(self):
        ...
        assert any(
            keyword in content.lower() for keyword in ["today", "right now", "asap"]
        )
    ...
    async def test_classify_urgency_length_factor(self):
        ...
        medium_content = (
            "I have an urgent problem that needs immediate attention please help me."
        )
    ...
    def test_analyze_keywords_medium_urgency(self):
        ...
        content = (
            "I have a problem with my order and I'm having trouble with the service."
        )
    ...
    async def test_validate_urgency_classification_inconsistent_rating(self):
        ...
        assert any(
            "inconsistent" in issue.lower() for issue in result["validation_issues"]
        )
    ...
    async def test_validate_urgency_classification_short_content(self):
        ...
        assert result["is_valid"] == False
        assert any(
            "short content" in issue.lower() for issue in result["validation_issues"]
        )
    ...
    async def test_validate_urgency_classification_no_keywords(self):
        ...
        content = (
            "The meal was not what I expected and the atmosphere was disappointing."
        )
        ...
        if not result["is_valid"]:
            assert any(
                "keyword evidence" in issue.lower()
    ...
    async def test_urgency_consistency(self):
        ...
        result1 = await UrgencyService.classify_urgency(
            content, rating, sentiment_score
        )
        result2 = await UrgencyService.classify_urgency(
            content, rating, sentiment_score
        )
    ...
    async def test_urgency_complex_scenarios(self):
        ...
        result = await UrgencyService.classify_urgency(
            content, rating=1, sentiment_score=0.1
        )
